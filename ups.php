...
function getRedirectUrl($baseUrl, $currentParams)
{
    ...
    if (!empty($currentParams['processor'])) { // This now refers to ups_brand (e.g., specific type of accessory brand)
...
// Updated title for UPS
$title = 'UPS Price in Pakistan | UPS Battery | Online UPS';
...
// Changed table name to 'ups' in sort order clauses
$sort_order = "ups.price ASC";
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "ups.price DESC";
            break;
        case "low":
            $sort_order = "ups.price ASC";
            break;
        case "rating":
            $sort_order = "ups.review DESC";
            break;
        default:
            $sort_order = "ups.price ASC";
    }
...
$selected_brand = isset($_GET['brand']) ? $_GET['brand'] : ''; // This refers to ups_store
$selected_processor = isset($_GET['processor']) ? urldecode($_GET['processor']) : ''; // This refers to ups_brand
...
if (!empty($selected_brand)) {
    ...
    $where_clauses[] = "ups_store.upsstore = ?";
...
if (!empty($selected_processor)) {
    ...
    $where_clauses[] = "ups_brand.upsbrand = ?";
...
$total_products_query = "SELECT COUNT(*) as total
                         FROM ups
                         INNER JOIN ups_store ON ups.brand = ups_store.upsstore
                         INNER JOIN ups_brand ON ups.processor = ups_brand.upsbrand"
    . $where_clause_string;
...
$product_query = "SELECT ups.*, ups_store.upsstore AS brand_name, ups_brand.upsbrand AS processor_name
                  FROM ups
                  INNER JOIN ups_store ON ups.brand = ups_store.upsstore
                  INNER JOIN ups_brand ON ups.processor = ups_brand.upsbrand"
    . $where_clause_string
...
<nav class="breadcrumb">
    <ul>
        ...
        <li>/</li>
        <li><a href="ups.php" class="active">UPS</a></li>
    </ul>
</nav>
...
<div class="ppcontainer">
    <aside class="sidebar">
        <div class="filter-category">
            <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> UPS Categories</button>
            <ul class="filter-list" style="display: none;">
                <?php
                // Changed table and column names for store categories
                $store_query = "SELECT DISTINCT upsstore FROM ups_store ORDER BY upsstore ASC";
        ...
        <div class="filter-category">
            ...
            <ul class="filter-list" style="display: none;">
                ...
                $brand_query = "SELECT DISTINCT upsbrand FROM ups_brand ORDER BY created_at DESC";
    ...
    <div class="main-content">
        <div class="product-grid">
            <div class="mainheading">
                <h3 id="mouse-heading">UPS <?php echo $selected_brand ? " >> " . htmlspecialchars($selected_brand) : ''; ?></h3>
            </div>
            <div class="pp">
                <p>Shop a wide selection of UPS in Pakistan including various models at Czone.com.pk</p>
            </div>
            ...
            <?php while ($row = mysqli_fetch_assoc($product_result)): {
                ...
                $brand = isset($row['brand_name']) ? htmlspecialchars($row['brand_name']) : 'N/A'; // From ups_store
                $processor = isset($row['processor_name']) ? htmlspecialchars($row['processor_name']) : 'N/A'; // From ups_brand
            } ?>
                <div class="product-container card" data-category="ups" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                    ...
                    <div class="product-details">
                        ...
                        <p class="category"><strong>Category:</strong> UPS > <?php echo $brand; ?></p>
                    ...
                    <div class="product-right">
                        ...
                        <?php endif; ?>
                        <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'ups')"> <i class="fa-solid fa-cart-shopping"></i> Add To Cart
                        </button>
        ...
        <div class="endpara">
            <p>Get precise, responsive control with our wide range of UPS for all your computing needs.</p>
            <p>From everyday use to professional applications, we've got you covered with trusted brands.</p>
            <p>Enjoy high-performance UPS, all at competitive prices in Pakistan.</p>
        </div>
    </div>
</div>

<div id="productPopup" class="popup" data-category="ups">
    <div class="popup-content">
        ...
        <div class="aside">
            ...
            <button class="add-to-cart" onclick="addToCart(0, 'ups')"> <i class="fa-solid fa-cart-shopping"></i> Add To Cart
            </button>
...
<?php
mysqli_close($conn); // Close connection here after all DB operations
