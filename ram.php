...
function getRedirectUrl($baseUrl, $currentParams) {
    ...
    if (!empty($currentParams['memory'])) {
        $params['memory'] = $currentParams['memory'];
    }
    if (!empty($currentParams['store'])) {
        $params['store'] = $currentParams['store'];
    }
...
$title = 'Memory - Ram Prices in Pakistan - Computerzone';
...
$sort_order = "ram.price ASC";

if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "ram.price DESC";
            break;
        case "low":
            $sort_order = "ram.price ASC";
            break;
        case "rating":
            $sort_order = "ram.review DESC";
            break;
        default:
            $sort_order = "ram.price ASC";
    }
...
$selected_memory_module = isset($_GET['memory']) ? $_GET['memory'] : '';
$selected_capacity_store = isset($_GET['store']) ? urldecode($_GET['store']) : '';

$where_clauses = [];
$params = [];
$types = '';

if (!empty($selected_memory_module)) {
    $where_clauses[] = "memory.module = ?";
    $params[] = $selected_memory_module;
    $types .= 's';
}

if (!empty($selected_capacity_store)) {
    $where_clauses[] = "r_capacity.capacity = ?";
    $params[] = $selected_capacity_store;
    $types .= 's';
}

if (!empty($searchQuery)) {
    $where_clauses[] = "ram.name LIKE ?";
    $params[] = '%' . $searchQuery . '%';
    $types .= 's';
}
...
$total_products_query = "SELECT COUNT(*) as total
                         FROM ram
                         INNER JOIN memory ON ram.memory = memory.module
                         INNER JOIN r_capacity ON ram.store = r_capacity.capacity"
                         . $where_clause_string;
...
$product_query = "SELECT ram.*, memory.module AS memory_module_name, r_capacity.capacity AS capacity_name
                  FROM ram
                  INNER JOIN memory ON ram.memory = memory.module
                  INNER JOIN r_capacity ON ram.store = r_capacity.capacity"
                  . $where_clause_string
...
?>
    ...
    <nav class="breadcrumb">
        <ul>
            ...
            <li>/</li>
            <li><a href="ram.php" class="active">Memory Module / RAM</a></li>
        </ul>
    </nav>
    ...
    <div class="ppcontainer">
        ...
        <aside class="sidebar">
            ...
            <div class="filter-category">
                <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> RAM Types</button>
                <ul class="filter-list" style="display: none;">
                    <?php
                    $memory_query = "SELECT DISTINCT module FROM memory ORDER BY created_at DESC";
                    $memory_result = mysqli_query($conn, $memory_query);

                    if (!$memory_result) {
                        echo "<li>Error fetching RAM options: " . htmlspecialchars(mysqli_error($conn)) . "</li>";
                    } else {
                        while ($row = mysqli_fetch_assoc($memory_result)) {
                            $original_value = $row['module'];
                            $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                            $url_value = urlencode($original_value);
                            $is_active = (isset($_GET['memory']) && urldecode($_GET['memory']) === $original_value) ? 'active' : '';
                            echo "<li class='$is_active'><a href='?memory=$url_value'>$display_value</a></li>";
                        }
                    }
                    ?>
                </ul>
            ...
            <div class="filter-category">
                ...
                <ul class="filter-list" style="display: none;">
                    <?php
                    $capacity_query = "SELECT DISTINCT capacity FROM r_capacity ORDER BY created_at DESC";
                    ...
                    if (!$capacity_result) {
                        ...
                        while ($row = mysqli_fetch_assoc($capacity_result)) {
                            $original_value = $row['capacity'];
                            $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                            $url_value = urlencode($original_value);
                            $is_active = (isset($_GET['store']) && urldecode($_GET['store']) === $original_value) ? 'active' : '';
                            echo "<li class='$is_active'><a href='?store=$url_value'>$display_value</a></li>";
                        }
                    }
                    ?>
                </ul>
        ...
        <div class="main-content">
            <div class="product-grid">
                <div class="mainheading">
                    <h3 id="ram-heading">Memory Module / RAM<?php echo $selected_memory_module ? " >> " . htmlspecialchars($selected_memory_module) : ''; ?></h3>
                </div>
                <div class="pp">
                    <p>Shop a wide selection of high-performance RAM modules in Pakistan including DDR3, DDR4, and DDR5 at Czone.com.pk</p>
                </div>
                ...
                <?php while ($row = mysqli_fetch_assoc($product_result)): {
                        $memory = isset($row['memory']) ? htmlspecialchars($row['memory']) : 'N/A';
                        $capacity = isset($row['store']) ? htmlspecialchars($row['store']) : 'N/A';
                    } ?>
                    <div class="product-container card" data-category="ram" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                        ...
                        <div class="product-details">
                            ...
                            <p class="category"><strong>Category:</strong> Memory Module / RAM > <?php echo $memory; ?></p>
                        ...
                        <div class="product-right">
                            ...
                            <?php endif; ?>
                            <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'ram')">
            ...
            <!-- End Paragraph -->
            <div class="endpara">
                <p>Upgrade your system's performance with our premium selection of RAM modules from top brands like Corsair, Kingston, G.Skill, and Crucial.</p>
                <p>From high-speed DDR4 for gaming rigs to reliable DDR3 for older systems, we offer memory solutions for every need and budget. Faster RAM means quicker data access, smoother multitasking, and better overall system responsiveness.</p>
                <p>Find the perfect RAM upgrade with various capacities and speeds at competitive prices in Pakistan.</p>
            </div>
        </div>
    </div>

    <!-- Product Popup -->
    <div id="productPopup" class="popup" data-category="ram">
        <div class="popup-content">
            ...
            <div class="aside">
                ...
                <button class="add-to-cart" onclick="addToCart(0, 'ram')">
                    <i class="fa-solid fa-cart-shopping"></i> Add To Cart
                </button>
