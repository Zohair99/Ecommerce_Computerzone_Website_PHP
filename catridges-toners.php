...
$title = 'Categories & Toners Price in Pakistan';
...
$sort_order = "carttrones.price ASC";
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "carttrones.price DESC";
            break;
        case "low":
            $sort_order = "carttrones.price ASC";
            break;
        case "rating":
            $sort_order = "carttrones.review DESC";
            break;
    }
}

$selected_brand = isset($_GET['brand']) ? mysqli_real_escape_string($conn, $_GET['brand']) : '';
$selected_type = isset($_GET['processor']) ? mysqli_real_escape_string($conn, urldecode($_GET['processor'])) : ''; // 'processor' often means 'type' or 'category' for cartridges/toners

$base_product_query = "SELECT carttrones.*, carttrones_store.cattronesstore AS brand_name, carttrones_brand.carttronesbrand AS type_name
                       FROM carttrones
                       INNER JOIN carttrones_store ON carttrones.brand = carttrones_store.cattronesstore
                       INNER JOIN carttrones_brand ON carttrones.processor = carttrones_brand.carttronesbrand"; // Assuming 'processor' column in carttrones links to 'carttronesbrand' in carttrones_brand

$base_count_query = "SELECT COUNT(*) as total
                     FROM carttrones
                     INNER JOIN carttrones_store ON carttrones.brand = carttrones_store.cattronesstore
                     INNER JOIN carttrones_brand ON carttrones.processor = carttrones_brand.carttronesbrand";

$conditions = [];
if (!empty($selected_brand)) {
    $conditions[] = "carttrones_store.cattronesstore = '" . $selected_brand . "'";
}
if (!empty($selected_type)) {
    $conditions[] = "carttrones_brand.carttronesbrand = '" . $selected_type . "'";
}
if (!empty($searchQuery)) {
    $conditions[] = "carttrones.name LIKE '%" . $searchQuery . "%'";
}
...
<nav class="breadcrumb">
    <ul>
        ...
        <li>/</li>
        <li><a href="catridges-toners.php" class="active">Cartridges & Toners</a></li>
    </ul>
</nav>
...
<div class="ppcontainer">
    <!-- Left Sidebar -->
    <aside class="sidebar">
        <!-- Cartridges & Toners Filter -->
        <div class="filter-category">
            <button class="filter-toggle">
                <i class="fa-solid fa-caret-down"></i> Cartridges & Toners
            </button>
            <ul class="filter-list" style="display: none;">
                <?php
                $store_query = "SELECT DISTINCT cattronesstore FROM carttrones_store ORDER BY cattronesstore ASC";
                ...
                if (!$store_result) {
                    ...
                    while ($row = mysqli_fetch_assoc($store_result)) {
                        $original_value = $row['cattronesstore'];
        ...
        <div class="filter-category">
            <button class="filter-toggle">
                <i class="fa-solid fa-caret-down"></i> Brand
            </button>
            <ul class="filter-list" style="display: none;">
                <?php
                $brand_query = "SELECT DISTINCT carttronesbrand FROM carttrones_brand ORDER BY created_at DESC";
                ...
                if (!$brand_result) {
                    ...
                    while ($row = mysqli_fetch_assoc($brand_result)) {
                        $original_value = $row['carttronesbrand'];
                        $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                        $url_value = urlencode($original_value);
    ...
    <div class="main-content">
        <div class="product-grid">
            <div class="mainheading">
                <h3 id="laptop-heading">Cartridges & Toners<?php echo $selected_brand ? " >> " . htmlspecialchars($selected_brand) : ''; ?></h3>
            </div>
            <div class="pp">
                <p>Shop a wide selection of Cartridges & Toners in Pakistan including various models at Czone.com.pk</p>
            </div>
            ...
            <?php while ($row = mysqli_fetch_assoc($product_result)): {
                ...
                <div class="product-container card" data-category="cartridges-toners" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                    ...
                    <div class="product-details">
                        ...
                        <p class="category"><strong>Category:</strong> Cartridges & Toners > <?php echo $brand; ?></p>
                    ...
                    <div class="product-right">
                        ...
                        <?php } ?>
                        <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'carttrones')">
        ...
        <!-- End Paragraph -->
        <div class="endpara">
            <p>Get consistent, high-quality prints every time with our wide range of cartridges and toners.</p>
...
<!-- Product Popup -->
<div id="productPopup" class="popup" data-category="cartridges-toners">
    <div class="popup-content">
        ...
        <div class="aside">
            ...
            <button class="add-to-cart" onclick="addToCart(0, 'carttrones')">
                <i class="fa-solid fa-cart-shopping"></i> Add To Cart
            </button>
