...
if (isset($_POST['btn'])) {
    ...
    $store_category = mysqli_real_escape_string($conn, $_POST['store_dropdown']); // This will be the usedproductstore
    $usedproduct_brand = mysqli_real_escape_string($conn, $_POST['brand_dropdown']); // This will be the usedproductbrand
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($temp, $final)) {
            ...
            // Changed table name to 'usedproduct' and column names for brand and processor
            $insert_query = "INSERT INTO usedproduct (offer, name, review, code, description, image, brand, processor, spec1, spec2, spec3, spec4, price, old_price)
                             VALUES ('$offer', '$name', '$review', '$code', '$description', '$file_name', '$store_category', '$usedproduct_brand', '$spec1', '$spec2', '$spec3', '$spec4', '$price', '$old_price')";

            if (mysqli_query($conn, $insert_query)) {
                echo "<p style='color: green;'>Used product record inserted successfully.</p>"; // Updated success message
            } else {
...
// Fetch dropdown values for usedproduct_brand and usedproduct_store
$brand_query = "SELECT id, usedproductbrand FROM usedproduct_brand ORDER BY usedproductbrand ASC"; // Changed table and column
$brand_result = mysqli_query($conn, $brand_query);

$store_query = "SELECT id, usedproductstore FROM usedproduct_store ORDER BY usedproductstore ASC"; // Changed table and column
...
<head>
    <meta charset="UTF-8">
    <title>Insert Used Product Data</title>
...
<body>
    <h2>Insert New Used Product</h2>
    <form action="" method="POST" enctype="multipart/form-data">
        <label for="name">Used Product Name</label>
        ...
        <select name="store_dropdown" id="store_dropdown" required>
            ...
            <?php while ($row = mysqli_fetch_assoc($store_result)) {
                // Changed column name to 'usedproductstore'
                echo "<option value='" . htmlspecialchars($row['usedproductstore']) . "'>" . htmlspecialchars($row['usedproductstore']) . "</option>";
            } ?>
        </select>

        <label for="brand_dropdown">Select Used Product Brand</label>
        <select name="brand_dropdown" id="brand_dropdown" required>
            <option selected disabled>Select a Brand</option>
            <?php while ($row = mysqli_fetch_assoc($brand_result)) {
                // Changed column name to 'usedproductbrand'
                echo "<option value='" . htmlspecialchars($row['usedproductbrand']) . "'>" . htmlspecialchars($row['usedproductbrand']) . "</option>";
            } ?>
        </select>
        ...
        <button type="submit" name="btn">Insert Used Product</button>
    </form>
</body>
</html>
