...
if (isset($_POST['update_product']) && isset($_POST['update_id'])) {
    ...
    // Update query for peripheralsmics table
    $update_query = "UPDATE peripheralsmics SET
                     offer='$offer',
                     name='$name',
                     review='$review',
                     code='$codee',
                     ...
                     image='$file_name',
                     brand='$brand',          -- This is the peripheralsmicsstore name
                     processor='$processor',  -- This is the peripheralsmicsbrand name
                     spec1='$spec1',
                     spec2='$spec2',
                     spec3='$spec3',
                     spec4='$spec4',
    ...
    if (mysqli_query($conn, $update_query)) {
        echo "<script>alert('Product updated successfully.'); window.location.href='view_peripherals-mics.php';</script>"; // Redirect to your peripheralsmics view page
    } else {
        die("SQL Error in Update: " . mysqli_error($conn));
    }
}

// Fetch product details for editing from peripheralsmics table
$edit_data = null;
if (isset($_GET['edit_id'])) {
    $edit_id = intval($_GET['edit_id']);
    $edit_query = "SELECT * FROM peripheralsmics WHERE id='$edit_id'";
...
<head>
    ...
    <title>Edit Peripheral/Misc Product</title>
    <style>
        body {
...
<body>
    <?php if ($edit_data) { ?>
        <h2>Edit Peripheral / Miscellaneous Product</h2>
        <form method="POST" enctype="multipart/form-data">
            ...
            <label>Brand (e.g., Logitech, Razer):</label>
            <select name="processor_dropdown" required>
                <option value="">Select Brand</option>
                <?php
                // Reset the pointer of the brand result set (peripheralsmics_brand)
                mysqli_data_seek($brand_result, 0);
                while ($Brand = mysqli_fetch_assoc($brand_result)) { ?>
                    <option value="<?php echo htmlspecialchars($Brand['peripheralsmicsbrand']); ?>"
                        <?php if ($edit_data['processor'] == $Brand['peripheralsmicsbrand']) echo 'selected'; ?>>
                        <?php echo htmlspecialchars($Brand['peripheralsmicsbrand']); ?>
                    </option>
                <?php } ?>
            </select>

            <label>Category / Store (e.g., Keyboard, Mouse, Headset):</label>
            <select name="store_dropdown" required>
                <option value="">Select Category / Store</option>
                <?php
                // Reset the pointer of the store result set (peripheralsmics_store)
                mysqli_data_seek($store_result, 0);
                while ($Store = mysqli_fetch_assoc($store_result)) { ?>
                    <option value="<?php echo htmlspecialchars($Store['peripheralsmicsstore']); ?>"
                        <?php if ($edit_data['brand'] == $Store['peripheralsmicsstore']) echo 'selected'; ?>>
                        <?php echo htmlspecialchars($Store['peripheralsmicsstore']); ?>
                    </option>
                <?php } ?>
            </select>
            ...
            <input type="text" name="codee" value="<?php echo htmlspecialchars($edit_data['code']); ?>" required> <label>Product Image:</label>
            ...
            <label>Price:</label>
            <input type="number" name="price" value="<?php echo htmlspecialchars($edit_data['price']); ?>" step="0.01" required> <label>Old Price:</label>
            <input type="number" name="old_price" value="<?php echo htmlspecialchars($edit_data['old_price']); ?>" step="0.01"> <button type="submit" name="update_product">Update Product</button>
        </form>
    <?php } else { ?>
