...
$store_result = mysqli_query($conn, "SELECT * FROM motherboard_store");
$brand_result = mysqli_query($conn, "SELECT * FROM motherboard_brand");
...
if (isset($_POST['update_product']) && isset($_POST['update_id'])) {
    ...
    // Check if brand and processor exist (foreign key safe)
    $brand_check = mysqli_query($conn, "SELECT motherboardstore FROM motherboard_store WHERE motherboardstore = '$brand'");
    $processor_check = mysqli_query($conn, "SELECT motherboardbrand FROM motherboard_brand WHERE motherboardbrand = '$processor'");

    if (mysqli_num_rows($brand_check) === 0 || mysqli_num_rows($processor_check) === 0) {
        die("<p style='color:red;'>Invalid brand or processor selected. Ensure both exist in the reference tables.</p>");
    }

    $update_query = "UPDATE motherboard SET 
                    offer='$offer',
                    name='$name',
                    review='$review',
                    code='$code',
    ...
    if (mysqli_query($conn, $update_query)) {
        echo "<script>alert('Product updated successfully.'); window.location.href='view_motherboard.php';</script>";
    } else {
        die("SQL Error in Update: " . mysqli_error($conn));
    }
}

// Fetch motherboard product
$edit_data = null;
if (isset($_GET['edit_id'])) {
    $edit_id = intval($_GET['edit_id']);
    $edit_result = mysqli_query($conn, "SELECT * FROM motherboard WHERE id='$edit_id'");
...
<?php if ($edit_data) { ?>
<h2>Edit Motherboard Product</h2>
<form method="POST" enctype="multipart/form-data">
    ...
    <select name="brand_dropdown" required>
        ...
        <?php mysqli_data_seek($store_result, 0); while ($row = mysqli_fetch_assoc($store_result)) {
            $val = $row['motherboardstore'];
    ...
    <select name="processor_dropdown" required>
        ...
        <?php mysqli_data_seek($brand_result, 0); while ($row = mysqli_fetch_assoc($brand_result)) {
            $val = $row['motherboardbrand'];
