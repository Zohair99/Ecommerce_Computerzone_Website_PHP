...
if (isset($_POST['btn'])) {
    ...
    $brand = mysqli_real_escape_string($conn, $_POST['brand_dropdown']); // from headsetheadphonemic_store
    $processor = mysqli_real_escape_string($conn, $_POST['processor_dropdown']); // from headsetheadphonemic_brand
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($temp, $final)) {
            ...
            $insert_query = "INSERT INTO headsetheadphonemic 
            ...
            if (mysqli_query($conn, $insert_query)) {
                echo "<p style='color: green;'>Headset/Headphone/Mic inserted successfully.</p>";
            } else {
...
// Fetch dropdown data
$brand_result = mysqli_query($conn, "SELECT * FROM headsetheadphonemic_store");
$processor_result = mysqli_query($conn, "SELECT * FROM headsetheadphonemic_brand");
?>
...
<head>
    <meta charset="UTF-8">
    <title>Add Headset/Headphone/Mic</title>
    <style>
...
<body>
    <h2>Insert Headset / Headphone / Mic</h2>
