...
if (isset($_POST['btn'])) {
    ...
    $brand = mysqli_real_escape_string($conn, $_POST['brand_dropdown']); // from memorycards_store
    $processor = mysqli_real_escape_string($conn, $_POST['processor_dropdown']); // from memorycards_brand
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($temp, $final)) {
            ...
            $insert_query = "INSERT INTO memorycards 
            ...
            if (mysqli_query($conn, $insert_query)) {
                echo "<p style='color: green;'>Memory card product inserted successfully.</p>";
            } else {
...
// Fetch dropdown data
$brand_result = mysqli_query($conn, "SELECT * FROM memorycards_store");
$processor_result = mysqli_query($conn, "SELECT * FROM memorycards_brand");
?>
...
<head>
    <meta charset="UTF-8">
    <title>Add Memory Card Product</title>
    <style>
...
<body>
    <h2>Insert Memory Card Product</h2>
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <label for="brand_dropdown">Select Memory Card Store</label>
        <select name="brand_dropdown" required>
            <option value="" disabled selected>Select a store</option>
            <?php while ($row = mysqli_fetch_assoc($brand_result)) {
                $val = htmlspecialchars($row['memorycardsstore']);
                echo "<option value='$val'>$val</option>";
            } ?>
        </select>

        <label for="processor_dropdown">Select Memory Card Brand</label>
        <select name="processor_dropdown" required>
            <option value="" disabled selected>Select a brand</option>
            <?php while ($row = mysqli_fetch_assoc($processor_result)) {
                $val = htmlspecialchars($row['memorycardsbrand']);
                echo "<option value='$val'>$val</option>";
            } ?>
        </select>
