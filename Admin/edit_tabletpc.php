...
// Fetch brands for the dropdown (from tabletpc_brand)
$brand_query = "SELECT id, tabletpcbrand FROM tabletpc_brand ORDER BY tabletpcbrand ASC";
$brand_result = mysqli_query($conn, $brand_query);

// Fetch stores for the dropdown (from tabletpc_store)
$store_query = "SELECT id, tabletpcstore FROM tabletpc_store ORDER BY tabletpcstore ASC";
...
if (isset($_POST['update_product']) && isset($_POST['update_id'])) {
    ...
    $update_query = "UPDATE tabletpc SET
                     offer='$offer',
                     name='$name',
                     review='$review',
                     code='$codee',
    ...
    if (mysqli_query($conn, $update_query)) {
        echo "<script>alert('Tablet PC product updated successfully.'); window.location.href='view_tabletpc.php';</script>";
    } else {
...
if (isset($_GET['edit_id'])) {
    ...
    $edit_query = "SELECT * FROM tabletpc WHERE id='$edit_id'";
...
<head>
    <meta charset="UTF-8">
    <title>Edit Tablet PC Product</title>
    <style>
        body {
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <select name="processor_dropdown" required>
            ...
            while ($Brand = mysqli_fetch_assoc($brand_result)) { ?>
                <option value="<?= htmlspecialchars($Brand['tabletpcbrand']); ?>"
                    <?= ($edit_data['processor'] == $Brand['tabletpcbrand']) ? 'selected' : ''; ?>>
                    <?= htmlspecialchars($Brand['tabletpcbrand']); ?>
                </option>
            <?php } ?>
        </select>
        ...
        <select name="store_dropdown" required>
            ...
            while ($Store = mysqli_fetch_assoc($store_result)) { ?>
                <option value="<?= htmlspecialchars($Store['tabletpcstore']); ?>"
                    <?= ($edit_data['brand'] == $Store['tabletpcstore']) ? 'selected' : ''; ?>>
                    <?= htmlspecialchars($Store['tabletpcstore']); ?>
                </option>
            <?php } ?>
        </select>
        ...
        <img src="images/<?= htmlspecialchars($edit_data['image']); ?>" alt="Tablet PC Image">
...
<?php } else { ?>
    <p class="error">Tablet PC product not found. Please provide a valid `edit_id` in the URL.</p>
<?php } ?>
</body>
</html>
