...
if (isset($_POST['btn'])) {
    ...
    $brand = mysqli_real_escape_string($conn, $_POST['brand_dropdown']); // from cooling_store
    $processor = mysqli_real_escape_string($conn, $_POST['processor_dropdown']); // from cooling_brand
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($temp, $final)) {
            ...
            $insert_query = "INSERT INTO cooling 
            ...
            if (mysqli_query($conn, $insert_query)) {
                echo "<p style='color: green;'>Cooling product inserted successfully.</p>";
            } else {
...
// Fetch dropdown data
$brand_result = mysqli_query($conn, "SELECT * FROM cooling_store");
$processor_result = mysqli_query($conn, "SELECT * FROM cooling_brand");
?>
...
<head>
    <meta charset="UTF-8">
    <title>Add Cooling Product</title>
    <style>
