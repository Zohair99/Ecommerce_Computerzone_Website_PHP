...
// Fetch np and type for the dropdowns
...
if (isset($_POST['update_product']) && isset($_POST['update_id'])) {
    ...
    $np = mysqli_real_escape_string($conn, $_POST['np_dropdown']); // Use np name
    $type = mysqli_real_escape_string($conn, $_POST['type_dropdown']); // Use type name
    ...
    // Update query
    $update_query = "UPDATE network SET 
                    offer='$offer',
                    name='$name',
                    review='$review',
                    code='$code',
    ...
    if (mysqli_query($conn, $update_query)) {
        echo "<script>alert('Product updated successfully.'); window.location.href='view_network.php';</script>";
    } else {
...
if (isset($_GET['edit_id'])) {
    ...
    $edit_query = "SELECT * FROM network WHERE id='$edit_id'";
...
<?php if ($edit_data) { ?>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <label>Network:</label>
        <select name="np_dropdown" required>
            <option value="">Select Network</option>
            <?php
            mysqli_data_seek($np_result, 0);
            while ($np = mysqli_fetch_assoc($np_result)) { ?>
                <option value="<?php echo $np['network']; ?>" 
                    <?php if (isset($edit_data['np']) && $edit_data['np'] == $np['network']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($np['network']); ?>
                </option>
            <?php } ?>
        </select>

        <label>Type:</label>
        <select name="type_dropdown" required>
            <option value="">Select Type</option>
            <?php
            mysqli_data_seek($type_result, 0);
            while ($type = mysqli_fetch_assoc($type_result)) { ?>
                <option value="<?php echo $type['n_type']; ?>" 
                    <?php if (isset($edit_data['type']) && $edit_data['type'] == $type['n_type']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($type['n_type']); ?>
                </option>
            <?php } ?>
        </select>
