...
if (isset($_POST['btn'])) {
    ...
    $software_brand = mysqli_real_escape_string($conn, $_POST['brand_dropdown']);
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($temp, $final)) {
            ...
            $insert_query = "INSERT INTO software (offer, name, review, code, description, image, brand, processor, spec1, spec2, spec3, spec4, price, old_price)
                             VALUES ('$offer', '$name', '$review', '$code', '$description', '$file_name', '$store_category', '$software_brand', '$spec1', '$spec2', '$spec3', '$spec4', '$price', '$old_price')";

            if (mysqli_query($conn, $insert_query)) {
                echo "<p style='color: green;'>Software record inserted successfully.</p>";
            } else {
...
<head>
    <meta charset="UTF-8">
    <title>Insert Software Data</title>
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <select name="store_dropdown" id="store_dropdown" required>
            ...
            <?php while ($row = mysqli_fetch_assoc($store_result)) {
                echo "<option value='" . htmlspecialchars($row['softwarestore']) . "'>" . htmlspecialchars($row['softwarestore']) . "</option>";
            } ?>
        </select>

        <label for="brand_dropdown">Select Software Brand</label>
        <select name="brand_dropdown" id="brand_dropdown" required>
            <option selected disabled>Select a Brand</option>
            <?php while ($row = mysqli_fetch_assoc($brand_result)) {
                echo "<option value='" . htmlspecialchars($row['softwarebrand']) . "'>" . htmlspecialchars($row['softwarebrand']) . "</option>";
            } ?>
        </select>
        ...
        <button type="submit" name="btn">Insert Software</button>
    </form>
</body>
</html>
