...
if (isset($_POST['btn'])) {
    ...
    $brand = mysqli_real_escape_string($conn, $_POST['brand_dropdown']); // from gamingproducts_store
    $processor = mysqli_real_escape_string($conn, $_POST['processor_dropdown']); // from gamingproducts_brand
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($temp, $final)) {
            ...
            $insert_query = "INSERT INTO gamingproducts 
            ...
            if (mysqli_query($conn, $insert_query)) {
                echo "<p style='color: green;'>Gaming product inserted successfully.</p>";
            } else {
...
// Fetch dropdown data
$brand_result = mysqli_query($conn, "SELECT * FROM gamingproducts_store");
$processor_result = mysqli_query($conn, "SELECT * FROM gamingproducts_brand");
?>
...
<body>
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <label for="brand_dropdown">Select Gaming Product Store</label>
        <select name="brand_dropdown" required>
            <option value="" disabled selected>Select a store</option>
            <?php while ($row = mysqli_fetch_assoc($brand_result)) {
                $val = htmlspecialchars($row['gamingproductsstore']);
                echo "<option value='$val'>$val</option>";
            } ?>
        </select>

        <label for="processor_dropdown">Select Gaming Product Brand</label>
        <select name="processor_dropdown" required>
            <option value="" disabled selected>Select a brand</option>
            <?php while ($row = mysqli_fetch_assoc($processor_result)) {
                $val = htmlspecialchars($row['gamingproductsbrand']);
                echo "<option value='$val'>$val</option>";
            } ?>
        </select>
