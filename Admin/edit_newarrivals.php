...
if (isset($_POST['update_product']) && isset($_POST['update_id'])) {
    ...
    if (!empty($_FILES['image']['name'])) {
        if ($_FILES['image']['error'] == 0) {
            echo "Image upload successful: " . $_FILES['image']['name'] . "<br>";
            $temp = $_FILES['image']['tmp_name'];
            $file_name = $_FILES['image']['name'];
            $final = 'images/' . $file_name;
            move_uploaded_file($temp, $final);
        } else {
            echo "Image upload failed: " . $_FILES['image']['error'] . "<br>";
        }
    } else {
        $file_name = $_POST['existing_image'];
    }

    // Update query with debug
    $update_query = "UPDATE New_Home SET 
                    offer='$offer', 
                    name='$name', 
    ...
    echo "Executing query: $update_query <br>";

    if (mysqli_query($conn, $update_query)) {
        echo "Update successful!<br>";
        echo "<script>alert('Product updated successfully.'); window.location.href='view_newarrival.php';</script>";
    } else {
        die("SQL Error in Update: " . mysqli_error($conn) . "<br>Query: " . $update_query);
    }
}

// Fetch product details with debug
$edit_data = null;
if (isset($_GET['edit_id'])) {
    $edit_id = intval($_GET['edit_id']);
    echo "Debug: Edit ID = " . $edit_id . "<br>";
    
    $edit_query = "SELECT * FROM New_Home WHERE id='$edit_id'";
    $edit_result = mysqli_query($conn, $edit_query);
    
    if (!$edit_result) {
        die("SQL Error in Fetch: " . mysqli_error($conn));
    } elseif (mysqli_num_rows($edit_result) == 0) {
        die("No product found with ID: " . $edit_id);
    } else {
        echo "Product found: " . $edit_id . "<br>";
    }
    
    $edit_data = mysqli_fetch_assoc($edit_result);
} else {
    die("Error: edit_id not provided in GET request.");
}
