...
// Fetch brands for the dropdown (from usedproduct_brand)
$brand_query = "SELECT id, usedproductbrand FROM usedproduct_brand ORDER BY usedproductbrand ASC";
$brand_result = mysqli_query($conn, $brand_query);

// Fetch stores for the dropdown (from usedproduct_store)
$store_query = "SELECT id, usedproductstore FROM usedproduct_store ORDER BY usedproductstore ASC";
...
if (isset($_POST['update_product']) && isset($_POST['update_id'])) {
    ...
    // 'processor' in usedproduct table maps to 'usedproductbrand' (the brand of the used product)
    $processor = mysqli_real_escape_string($conn, $_POST['processor_dropdown']);
    // 'brand' in usedproduct table maps to 'usedproductstore' (the category/store of the used product)
    ...
    // Update query - Changed table name to 'usedproduct'
    $update_query = "UPDATE usedproduct SET
                     offer='$offer',
                     name='$name',
                     review='$review',
                     code='$codee',
                     ...
                     image='$file_name',
                     brand='$brand',       -- Corresponds to usedproductstore
                     processor='$processor', -- Corresponds to usedproductbrand
                     spec1='$spec1',
                     spec2='$spec2',
                     spec3='$spec3',
                     spec4='$spec4',
    ...
    if (mysqli_query($conn, $update_query)) {
        ...
        echo "<script>alert('Used product updated successfully.'); window.location.href='view_usedproduct.php';</script>";
    } else {
...
if (isset($_GET['edit_id'])) {
    ...
    // Changed table name to 'usedproduct'
    $edit_query = "SELECT * FROM usedproduct WHERE id='$edit_id'";
...
<?php if ($edit_data) { ?>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <label>Used Product Brand:</label>
        <select name="processor_dropdown" required>
            <option value="">Select Used Product Brand</option>
            <?php
            mysqli_data_seek($brand_result, 0); // Reset pointer for dropdown
            while ($Brand = mysqli_fetch_assoc($brand_result)) { ?>
                <option value="<?php echo htmlspecialchars($Brand['usedproductbrand']); ?>"
                    <?php if ($edit_data['processor'] == $Brand['usedproductbrand']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($Brand['usedproductbrand']); ?>
                </option>
            <?php } ?>
        </select>
        ...
        <select name="store_dropdown" required>
            ...
            while ($Store = mysqli_fetch_assoc($store_result)) { ?>
                <option value="<?php echo htmlspecialchars($Store['usedproductstore']); ?>"
                    <?php if ($edit_data['brand'] == $Store['usedproductstore']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($Store['usedproductstore']); ?>
                </option>
            <?php } ?>
        </select>
        ...
        <img src="images/<?php echo htmlspecialchars($edit_data['image']); ?>" alt="Used Product Image">
        ...
        <button type="submit" name="update_product">Update Used Product</button>
    </form>
<?php } else { ?>
    <p class="error">Used product not found. Please provide a valid `edit_id` in the URL.</p>
<?php } ?>
</body>
</html>