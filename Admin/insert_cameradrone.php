...
if (isset($_POST['btn'])) {
    ...
    $brand = mysqli_real_escape_string($conn, $_POST['brand_dropdown']); // from camdrone_store
    $processor = mysqli_real_escape_string($conn, $_POST['processor_dropdown']); // from cambrand_store
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($temp, $final)) {
            ...
            $insert_query = "INSERT INTO cameradrone 
            ...
            if (mysqli_query($conn, $insert_query)) {
                echo "<p style='color: green;'>Camera Drone inserted successfully.</p>";
            } else {
...
// Fetch dropdown data
$brand_result = mysqli_query($conn, "SELECT * FROM camdrone_store");
$processor_result = mysqli_query($conn, "SELECT * FROM cambrand_store");
?>
...
<body>
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <label for="brand_dropdown">Select Drone (from camdrone_store)</label>
        <select name="brand_dropdown" required>
            ...
            <?php while ($row = mysqli_fetch_assoc($brand_result)) {
                $val = htmlspecialchars($row['camdrone']);
                echo "<option value='$val'>$val</option>";
            } ?>
        </select>

        <label for="processor_dropdown">Select Brand (from cambrand_store)</label>
        <select name="processor_dropdown" required>
            <option value="" disabled selected>Select a brand</option>
            <?php while ($row = mysqli_fetch_assoc($processor_result)) {
                $val = htmlspecialchars($row['cambrand']);
                echo "<option value='$val'>$val</option>";
            } ?>
        </select>
