...
// Fetch memorycards store and brand for the dropdowns
$store_query = "SELECT * FROM memorycards_store";
$store_result = mysqli_query($conn, $store_query);

$brand_query = "SELECT * FROM memorycards_brand";
...
if (isset($_POST['update_product']) && isset($_POST['update_id'])) {
    ...
    $brand = mysqli_real_escape_string($conn, $_POST['brand_dropdown']); // Use brand name
    $processor = mysqli_real_escape_string($conn, $_POST['processor_dropdown']); // Use processor name
    ...
    // Update query
    $update_query = "UPDATE memorycards SET 
                    offer='$offer',
                    name='$name',
                    review='$review',
                    code='$code',
    ...
    if (mysqli_query($conn, $update_query)) {
        echo "<script>alert('Product updated successfully.'); window.location.href='view_memorycard.php';</script>";
    } else {
...
if (isset($_GET['edit_id'])) {
    ...
    $edit_query = "SELECT * FROM memorycards WHERE id='$edit_id'";
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <select name="brand_dropdown" required>
            ...
            while ($store = mysqli_fetch_assoc($store_result)) { ?>
                <option value="<?php echo $store['memorycardsstore']; ?>" 
                    <?php if (isset($edit_data['brand']) && $edit_data['brand'] == $store['memorycardsstore']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($store['memorycardsstore']); ?>
                </option>
            <?php } ?>
        </select>
        ...
        <select name="processor_dropdown" required>
            ...
            while ($brand = mysqli_fetch_assoc($brand_result)) { ?>
                <option value="<?php echo $brand['memorycardsbrand']; ?>" 
                    <?php if (isset($edit_data['processor']) && $edit_data['processor'] == $brand['memorycardsbrand']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($brand['memorycardsbrand']); ?>
                </option>
            <?php } ?>
        </select>
