...
if (isset($_POST['btn'])) {
    ...
    $store_category = mysqli_real_escape_string($conn, $_POST['store_dropdown']); // brand in tabletacc_store
    $tabletacc_brand = mysqli_real_escape_string($conn, $_POST['brand_dropdown']); // processor in tabletacc_brand
    $review = mysqli_real_escape_string($conn, $_POST['rating']);
    $code = intval($_POST['codee']);  // code is INT according to your schema
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($temp, $final)) {
            ...
            $insert_query = "INSERT INTO tabletacc (offer, name, review, code, description, image, brand, processor, spec1, spec2, spec3, spec4, price, old_price)
                             VALUES ('$offer', '$name', '$review', $code, '$description', '$file_name', '$store_category', '$tabletacc_brand', '$spec1', '$spec2', '$spec3', '$spec4', '$price', '$old_price')";

            if (mysqli_query($conn, $insert_query)) {
                echo "<p style='color: green;'>Tablet accessory record inserted successfully.</p>";
            } else {
...
<head>
    <meta charset="UTF-8">
    <title>Insert Tablet Accessory Data</title>
...
<body>
    <h2>Insert New Tablet Accessory</h2>
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <label for="store_dropdown">Select Store Category</label>
        <select name="store_dropdown" id="store_dropdown" required>
            ...
            <?php while ($row = mysqli_fetch_assoc($store_result)) {
                echo "<option value='" . htmlspecialchars($row['tabletaccstore']) . "'>" . htmlspecialchars($row['tabletaccstore']) . "</option>";
            } ?>
        </select>

        <label for="brand_dropdown">Select Brand</label>
        <select name="brand_dropdown" id="brand_dropdown" required>
            <option selected disabled>Select a Brand</option>
            <?php while ($row = mysqli_fetch_assoc($brand_result)) {
                echo "<option value='" . htmlspecialchars($row['tabletaccbrand']) . "'>" . htmlspecialchars($row['tabletaccbrand']) . "</option>";
            } ?>
        </select>
        ...
        <button type="submit" name="btn">Insert Tablet Accessory</button>
    </form>
</body>
</html>
