...
if (isset($_POST['btn'])) {
    ...
    $brand = mysqli_real_escape_string($conn, $_POST['brand_dropdown']); // from deskcomp_store
    $processor = mysqli_real_escape_string($conn, $_POST['processor_dropdown']); // from deskcomp_brand
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($temp, $final)) {
            ...
            $insert_query = "INSERT INTO deskcomp 
            ...
            if (mysqli_query($conn, $insert_query)) {
                echo "<p style='color: green;'>Deskcomp product inserted successfully.</p>";
            } else {
...
// Fetch dropdown data
$brand_result = mysqli_query($conn, "SELECT * FROM deskcomp_store");
$processor_result = mysqli_query($conn, "SELECT * FROM deskcomp_brand");
?>
...
<head>
    <meta charset="UTF-8">
    <title>Add Deskcomp Product</title>
    <style>
