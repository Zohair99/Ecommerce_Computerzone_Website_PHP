...
if (isset($_POST['btn'])) {
    ...
    $brand = mysqli_real_escape_string($conn, $_POST['brand_dropdown']); // from categories_store
    $processor = mysqli_real_escape_string($conn, $_POST['processor_dropdown']); // from categories_brand
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($temp, $final)) {
            ...
            if (mysqli_query($conn, $insert_query)) {
                echo "<p style='color: green;'>Category product inserted successfully.</p>";
            } else {
...
// Fetch dropdown data
$brand_result = mysqli_query($conn, "SELECT * FROM categories_store");
$processor_result = mysqli_query($conn, "SELECT * FROM categories_brand");
?>
...
<head>
    <meta charset="UTF-8">
    <title>Add Product to Categories</title>
    <style>
...
<body>
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <label for="brand_dropdown">Select Category (from categories_store)</label>
        <select name="brand_dropdown" required>
            <option value="" disabled selected>Select a category</option>
            <?php while ($row = mysqli_fetch_assoc($brand_result)) {
                $val = htmlspecialchars($row['cattrones']);
                echo "<option value='$val'>$val</option>";
            } ?>
        </select>

        <label for="processor_dropdown">Select Brand (from categories_brand)</label>
        <select name="processor_dropdown" required>
            <option value="" disabled selected>Select a brand</option>
            <?php while ($row = mysqli_fetch_assoc($processor_result)) {
                $val = htmlspecialchars($row['catbrand']);
                echo "<option value='$val'>$val</option>";
            } ?>
        </select>
