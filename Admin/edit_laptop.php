...
if (isset($_POST['update_product']) && isset($_POST['update_id'])) {
    ...
    $brand = intval($_POST['brand_dropdown']); // Use brand ID instead of name
    ...
    if (mysqli_query($conn, $update_query)) {
        echo "<script>alert('Product updated successfully.'); window.location.href='view_laptop.php';</script>";
    } else {
...
<?php if ($edit_data) { ?>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <select name="brand_dropdown" required>
            ...
            <?php
            // Reset the pointer of the brand result set
            mysqli_data_seek($brand_result, 0);
            while ($Brand = mysqli_fetch_assoc($brand_result)) { ?>
                <option value="<?php echo $Brand['id']; ?>" 
                    <?php if ($edit_data['brand'] == $Brand['id']) echo 'selected'; ?>>
