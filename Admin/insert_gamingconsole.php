...
if (isset($_POST['btn'])) {
    ...
    $brand = mysqli_real_escape_string($conn, $_POST['brand_dropdown']); // from gamingconsoles_store
    $processor = mysqli_real_escape_string($conn, $_POST['processor_dropdown']); // from gamingconsoles_brand
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($temp, $final)) {
            ...
            $insert_query = "INSERT INTO gamingconsoles 
            ...
            if (mysqli_query($conn, $insert_query)) {
                echo "<p style='color: green;'>Gaming console inserted successfully.</p>";
            } else {
...
// Fetch dropdown data
$brand_result = mysqli_query($conn, "SELECT * FROM gamingconsoles_store");
$processor_result = mysqli_query($conn, "SELECT * FROM gamingconsoles_brand");
?>
...
<head>
    <meta charset="UTF-8">
    <title>Add Gaming Console</title>
    <style>
...
<body>
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <label for="brand_dropdown">Select Gaming Console Store</label>
        <select name="brand_dropdown" required>
            <option value="" disabled selected>Select a store</option>
            <?php while ($row = mysqli_fetch_assoc($brand_result)) {
                $val = htmlspecialchars($row['gamingconsolesstore']);
                echo "<option value='$val'>$val</option>";
            } ?>
        </select>

        <label for="processor_dropdown">Select Gaming Console Brand</label>
        <select name="processor_dropdown" required>
            <option value="" disabled selected>Select a brand</option>
            <?php while ($row = mysqli_fetch_assoc($processor_result)) {
                $val = htmlspecialchars($row['gamingconsolesbrand']);
                echo "<option value='$val'>$val</option>";
            } ?>
        </select>
