...
if (isset($_POST['btn'])) {
    ...
    $store_category = mysqli_real_escape_string($conn, $_POST['store_dropdown']); // brand in speaker
    $speaker_brand = mysqli_real_escape_string($conn, $_POST['brand_dropdown']); // processor in speaker
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($temp, $final)) {
            ...
            $insert_query = "INSERT INTO speaker (offer, name, review, code, description, image, brand, processor, spec1, spec2, spec3, spec4, price, old_price)
                             VALUES ('$offer', '$name', '$review', '$code', '$description', '$file_name', '$store_category', '$speaker_brand', '$spec1', '$spec2', '$spec3', '$spec4', '$price', '$old_price')";

            if (mysqli_query($conn, $insert_query)) {
                echo "<p style='color: green;'>Speaker record inserted successfully.</p>";
            } else {
...
<head>
    <meta charset="UTF-8">
    <title>Insert Speaker Data</title>
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <select name="store_dropdown" id="store_dropdown" required>
            ...
            <?php while ($row = mysqli_fetch_assoc($store_result)) {
                echo "<option value='" . htmlspecialchars($row['speakerstore']) . "'>" . htmlspecialchars($row['speakerstore']) . "</option>";
            } ?>
        </select>

        <label for="brand_dropdown">Select Speaker Brand</label>
        <select name="brand_dropdown" id="brand_dropdown" required>
            <option selected disabled>Select a Brand</option>
            <?php while ($row = mysqli_fetch_assoc($brand_result)) {
                echo "<option value='" . htmlspecialchars($row['speakerbrand']) . "'>" . htmlspecialchars($row['speakerbrand']) . "</option>";
            } ?>
        </select>
        ...
        <button type="submit" name="btn">Insert Speaker</button>
    </form>
</body>
</html>
