...
// Fetch brands for the dropdown (from tvstream_brand)
$brand_query = "SELECT id, tvstreambrand FROM tvstream_brand ORDER BY tvstreambrand ASC";
$brand_result = mysqli_query($conn, $brand_query);

// Fetch stores for the dropdown (from tvstream_store)
$store_query = "SELECT id, tvstreamstore FROM tvstream_store ORDER BY tvstreamstore ASC";
...
if (isset($_POST['update_product']) && isset($_POST['update_id'])) {
    ...
    // 'processor' in tvstream table maps to 'tvstreambrand' (the brand of the TV stream device)
    $processor = mysqli_real_escape_string($conn, $_POST['processor_dropdown']);
    // 'brand' in tvstream table maps to 'tvstreamstore' (the category/store of the TV stream device)
    ...
    // Update query - Changed table name to 'tvstream'
    $update_query = "UPDATE tvstream SET
                      offer='$offer',
                      name='$name',
                      review='$review',
                      code='$codee',
                      ...
                      image='$file_name',
                      brand='$brand',      -- Corresponds to tvstreamstore
                      processor='$processor', -- Corresponds to tvstreambrand
                      spec1='$spec1',
                      spec2='$spec2',
                      spec3='$spec3',
                      spec4='$spec4',
    ...
    if (mysqli_query($conn, $update_query)) {
        ...
        echo "<script>alert('TV Stream product updated successfully.'); window.location.href='view_tvstream.php';</script>";
    } else {
...
if (isset($_GET['edit_id'])) {
    ...
    // Changed table name to 'tvstream'
    $edit_query = "SELECT * FROM tvstream WHERE id='$edit_id'";
...
<head>
    <meta charset="UTF-8">
    <title>Edit TV Stream Product</title> <style>
        body {
...
<body>
<?php if ($edit_data) { ?>
    <h2>Edit TV Stream Product</h2> <form method="POST" enctype="multipart/form-data">
        ...
        <label>TV Stream Brand:</label> <select name="processor_dropdown" required>
            <option value="">Select TV Stream Brand</option> <?php
            mysqli_data_seek($brand_result, 0); // Reset pointer for dropdown
            while ($Brand = mysqli_fetch_assoc($brand_result)) { ?>
                <option value="<?php echo htmlspecialchars($Brand['tvstreambrand']); ?>"
                    <?php if ($edit_data['processor'] == $Brand['tvstreambrand']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($Brand['tvstreambrand']); ?>
                </option>
            <?php } ?>
        </select>

        <label>Store Category:</label> <select name="store_dropdown" required>
            <option value="">Select Store Category</option> <?php
            mysqli_data_seek($store_result, 0); // Reset pointer for dropdown
            while ($Store = mysqli_fetch_assoc($store_result)) { ?>
                <option value="<?php echo htmlspecialchars($Store['tvstreamstore']); ?>"
                    <?php if ($edit_data['brand'] == $Store['tvstreamstore']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($Store['tvstreamstore']); ?>
                </option>
            <?php } ?>
        </select>
        ...
        <img src="images/<?php echo htmlspecialchars($edit_data['image']); ?>" alt="TV Stream Image"> <label>Specification 1:</label>
        ...
        <button type="submit" name="update_product">Update TV Stream Product</button> </form>
<?php } else { ?>
    <p class="error">TV Stream product not found. Please provide a valid `edit_id` in the URL.</p> <?php } ?>
</body>
</html>