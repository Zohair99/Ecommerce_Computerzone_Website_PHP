...
// Fetch brands for the dropdown (from scanners_brand)
$brand_query = "SELECT id, scannersbrand FROM scanners_brand ORDER BY scannersbrand ASC";
$brand_result = mysqli_query($conn, $brand_query);

// Fetch categories/stores for the dropdown (from scanners_store)
...
if (isset($_POST['update_product']) && isset($_POST['update_id'])) {
    ...
    $update_query = "UPDATE scanners SET
                     offer='$offer',
                     name='$name',
                     review='$review',
                     code='$codee',
    ...
    if (mysqli_query($conn, $update_query)) {
        echo "<script>alert('Scanner updated successfully.'); window.location.href='view_scanners.php';</script>";
    } else {
...
if (isset($_GET['edit_id'])) {
    ...
    $edit_query = "SELECT * FROM scanners WHERE id='$edit_id'";
...
<head>
    <meta charset="UTF-8">
    <title>Edit Scanner Product</title>
    <style>
        body {
...
<?php if ($edit_data) { ?>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <select name="processor_dropdown" required>
            <option value="">Select Scanner Brand</option>
            <?php
            mysqli_data_seek($brand_result, 0);
            while ($Brand = mysqli_fetch_assoc($brand_result)) { ?>
                <option value="<?php echo htmlspecialchars($Brand['scannersbrand']); ?>"
                    <?php if ($edit_data['processor'] == $Brand['scannersbrand']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($Brand['scannersbrand']); ?>
                </option>
            <?php } ?>
        </select>
        ...
        <select name="store_dropdown" required>
            ...
            while ($Store = mysqli_fetch_assoc($store_result)) { ?>
                <option value="<?php echo htmlspecialchars($Store['scannersstore']); ?>"
                    <?php if ($edit_data['brand'] == $Store['scannersstore']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($Store['scannersstore']); ?>
                </option>
            <?php } ?>
        </select>
        ...
        <img src="images/<?php echo htmlspecialchars($edit_data['image']); ?>" alt="Scanner Image">
...
<?php } else { ?>
    <p class="error">Scanner not found. Please provide a valid `edit_id` in the URL.</p>
<?php } ?>
</body>
</html>
