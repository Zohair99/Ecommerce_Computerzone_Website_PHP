...
if (isset($_POST['btn'])) {
    ...
    $model = mysqli_real_escape_string($conn, $_POST['model_dropdown']);
    $m_series = mysqli_real_escape_string($conn, $_POST['series_dropdown']);
    $review = mysqli_real_escape_string($conn, $_POST['rating']);
    $code = intval($_POST['codee']); 
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($temp, $final)) {
            ...
            $old_price = isset($_POST['old_price']) ? floatval($_POST['old_price']) : null; // Ensure old_price is a float

            // Insert query
            $insert_query = "INSERT INTO printer_data (offer, name, review, code, description, image, printers, series, spec1, spec2, spec3, spec4, price, old_price)
                             VALUES ('$offer', '$name', '$review', $code, '$description', '$file_name', '$model', '$m_series', '$spec1', '$spec2', '$spec3', '$spec4', $price, $old_price)";
...
$graphic_query = "SELECT * FROM printers";
$graphic_result = mysqli_query($conn, $graphic_query);

$chip_query = "SELECT * FROM series";
...
<body>
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <label for="model_dropdown">Select a model</label>
        <select class="form-select" name="model_dropdown" id="model_dropdown" required>
        ...
        <label for="series_dropdown">Select a series</label>
        <select class="form-select" name="series_dropdown" id="series_dropdown">
            <option selected disabled>Select a series</option>
            <?php
            while ($row = mysqli_fetch_assoc($chip_result)) {
                $m_series = $row['m_series'];
                echo "<option value='$m_series'>$m_series</option>";
            }
            ?>
        </select>
