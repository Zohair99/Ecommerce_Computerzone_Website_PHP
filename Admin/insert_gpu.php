...
if (isset($_POST['btn'])) {
    ...
    $gpu_name = mysqli_real_escape_string($conn, $_POST['graphic_dropdown']);
    $chip_set = mysqli_real_escape_string($conn, $_POST['chip_dropdown']);
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($temp, $final)) {
            ...
            // Insert query
            $insert_query = "INSERT INTO gpu (offer, name, review, codee, description, image, gpu_name, chip_set, spec1, spec2, spec3, spec4, price, old_price) 
                             VALUES ('$offer', '$name', '$review', '$codee', '$description', '$file_name', '$gpu_name', '$chip_set', '$spec1', '$spec2', '$spec3', '$spec4', '$price', '$old_price')";
...
$chip_query = "SELECT * FROM chipset";
...
<body>
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <label for="graphic_dropdown">Select a GPU</label>
        <select class="form-select" name="graphic_dropdown" id="graphic_dropdown" required>
            <option selected disabled>Select a GPU</option>
            <?php
            while ($row = mysqli_fetch_assoc($graphic_result)) {
                $gpu_name = $row['gpu_name'];
                echo "<option value='$gpu_name'>$gpu_name</option>";
            }
            ?>
        </select>

        <label for="chip_dropdown">Select a Chipset</label>
        <select class="form-select" name="chip_dropdown" id="chip_dropdown">
            <option selected disabled>Select a Chipset</option>
            <?php
            while ($row = mysqli_fetch_assoc($chip_result)) {
                $chip_set = $row['chip_set'];
                echo "<option value='$chip_set'>$chip_set</option>";
            }
            ?>
        </select>
