...
// Fetch monitors and sizes for the dropdowns
$monitor_query = "SELECT * FROM printers";
$monitor_result = mysqli_query($conn, $monitor_query);

$size_query = "SELECT * FROM series";
...
if (isset($_POST['update_product']) && isset($_POST['update_id'])) {
    ...
    $model = mysqli_real_escape_string($conn, $_POST['model_dropdown']); // Use monitor name
    $m_series = mysqli_real_escape_string($conn, $_POST['series_dropdown']); // Use size name
    ...
    // Update query
    $update_query = "UPDATE printer_data SET
                    offer='$offer',
                    name='$name',
                    review='$review',
                    code='$code',
    ...
    if (mysqli_query($conn, $update_query)) {
        echo "<script>alert('Product updated successfully.'); window.location.href='view_printer.php';</script>";
    } else {
...
if (isset($_GET['edit_id'])) {
    ...
    $edit_query = "SELECT * FROM printer_data WHERE id='$edit_id'";
...
<?php if ($edit_data) { ?>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <label>Monitor:</label>
        <select name="model_dropdown" required>
            <option value="">Select Monitor</option>
            <?php
            mysqli_data_seek($monitor_result, 0);
            while ($monitor = mysqli_fetch_assoc($monitor_result)) { ?>
                <option value="<?php echo $monitor['model']; ?>"
                    <?php if ($edit_data['printers'] == $monitor['model']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($monitor['model']); ?>
                </option>
            <?php } ?>
        </select>

        <label>Series:</label>
        <select name="series_dropdown" required>
            <option value="">Select Series</option>
            <?php
            mysqli_data_seek($size_result, 0);
            while ($size = mysqli_fetch_assoc($size_result)) { ?>
                <option value="<?php echo $size['m_series']; ?>"
                    <?php if ($edit_data['series'] == $size['m_series']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($size['m_series']); ?>
                </option>
            <?php } ?>
        </select>
