...
if (isset($_POST['btn'])) {
    ...
    $power = mysqli_real_escape_string($conn, $_POST['power_dropdown']); // Updated to 'power_dropdown'
    $wattage = mysqli_real_escape_string($conn, $_POST['wattage_dropdown']); // Updated to 'wattage_dropdown'
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($temp, $final)) {
            ...
            // Insert query for psu table
            $insert_query = "INSERT INTO psu (offer, name, review, code, description, image, power, wattage, spec1, spec2, spec3, spec4, price, old_price)
                             VALUES ('$offer', '$name', '$review', '$code', '$description', '$file_name', '$power', '$wattage', '$spec1', '$spec2', '$spec3', '$spec4', '$price', '$old_price')";
...
$wattage_query = "SELECT * FROM wattage";
...
<body>
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <label for="power_dropdown">Select a Power Supply</label> <!-- Updated to 'power_dropdown' -->
        <select class="form-select" name="power_dropdown" id="power_dropdown" required>
            <option selected disabled>Select a power supply</option>
            <?php
        ...
        <label for="wattage_dropdown">Select a Wattage</label> <!-- Updated to 'wattage_dropdown' -->
        <select class="form-select" name="wattage_dropdown" id="wattage_dropdown" required>
            <option selected disabled>Select a wattage</option>
            <?php
            while ($row = mysqli_fetch_assoc($wattage_result)) {
                $watt = $row['watt'];
                echo "<option value='$watt'>$watt</option>";
            }
            ?>
        </select>
