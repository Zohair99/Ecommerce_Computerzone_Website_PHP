...
if (isset($_POST['btn'])) {
    ...
    $brand = mysqli_real_escape_string($conn, $_POST['brand_dropdown']); // from laptopacc_store
    $processor = mysqli_real_escape_string($conn, $_POST['processor_dropdown']); // from brand_store
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($temp, $final)) {
            ...
            $insert_query = "INSERT INTO laptopacc 
            ...
            if (mysqli_query($conn, $insert_query)) {
                echo "<p style='color: green;'>Laptop record inserted successfully.</p>";
            } else {
...
// Fetch dropdown data
$brand_result = mysqli_query($conn, "SELECT * FROM laptopacc_store");
$processor_result = mysqli_query($conn, "SELECT * FROM brand_store");
?>
...
<head>
    <meta charset="UTF-8">
    <title>Add Laptop Accessory</title>
...
<body>
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <label for="brand_dropdown">Select Accessory (laptopacc)</label>
        <select name="brand_dropdown" required>
            ...
            <?php while ($row = mysqli_fetch_assoc($brand_result)) {
                $val = htmlspecialchars($row['laptopacc']);
        ...
        <select name="processor_dropdown" required>
            ...
            <?php while ($row = mysqli_fetch_assoc($processor_result)) {
                $val = htmlspecialchars($row['brand']);
                echo "<option value='$val'>$val</option>";
            } ?>
        </select>
