...
if (isset($_POST['btn'])) {
    ...
    $store_category = mysqli_real_escape_string($conn, $_POST['store_dropdown']); // This will be the upsstore
    $ups_brand = mysqli_real_escape_string($conn, $_POST['brand_dropdown']); // This will be the upsbrand
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($temp, $final)) {
            ...
            // Changed table name to 'ups' and column names for brand and processor
            $insert_query = "INSERT INTO ups (offer, name, review, code, description, image, brand, processor, spec1, spec2, spec3, spec4, price, old_price)
                             VALUES ('$offer', '$name', '$review', '$code', '$description', '$file_name', '$store_category', '$ups_brand', '$spec1', '$spec2', '$spec3', '$spec4', '$price', '$old_price')";

            if (mysqli_query($conn, $insert_query)) {
                echo "<p style='color: green;'>UPS record inserted successfully.</p>"; // Updated success message
            } else {
...
// Fetch dropdown values for ups_brand and ups_store
$brand_query = "SELECT id, upsbrand FROM ups_brand ORDER BY upsbrand ASC"; // Changed table and column
$brand_result = mysqli_query($conn, $brand_query);

$store_query = "SELECT id, upsstore FROM ups_store ORDER BY upsstore ASC"; // Changed table and column
...
<body>
    <h2>Insert New UPS Device</h2>
    <form action="" method="POST" enctype="multipart/form-data">
        <label for="name">UPS Device Name</label>
        ...
        <select name="store_dropdown" id="store_dropdown" required>
            ...
            <?php while ($row = mysqli_fetch_assoc($store_result)) {
                // Changed column name to 'upsstore'
                echo "<option value='" . htmlspecialchars($row['upsstore']) . "'>" . htmlspecialchars($row['upsstore']) . "</option>";
            } ?>
        </select>

        <label for="brand_dropdown">Select UPS Brand</label>
        <select name="brand_dropdown" id="brand_dropdown" required>
            <option selected disabled>Select a Brand</option>
            <?php while ($row = mysqli_fetch_assoc($brand_result)) {
                // Changed column name to 'upsbrand'
                echo "<option value='" . htmlspecialchars($row['upsbrand']) . "'>" . htmlspecialchars($row['upsbrand']) . "</option>";
            } ?>
        </select>
        ...
        <button type="submit" name="btn">Insert UPS Device</button>
    </form>
</body>
</html>
