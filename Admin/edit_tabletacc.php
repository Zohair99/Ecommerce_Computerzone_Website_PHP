...
// Fetch brands for the dropdown (from tabletacc_brand)
$brand_query = "SELECT id, tabletaccbrand FROM tabletacc_brand ORDER BY tabletaccbrand ASC";
$brand_result = mysqli_query($conn, $brand_query);

// Fetch stores for the dropdown (from tabletacc_store)
$store_query = "SELECT id, tabletaccstore FROM tabletacc_store ORDER BY tabletaccstore ASC";
...
if (isset($_POST['update_product']) && isset($_POST['update_id'])) {
    ...
    $update_query = "UPDATE tabletacc SET
                     offer='$offer',
                     name='$name',
                     review='$review',
                     code='$codee',
    ...
    if (mysqli_query($conn, $update_query)) {
        echo "<script>alert('Tablet accessory product updated successfully.'); window.location.href='view_tabletacc.php';</script>";
    } else {
...
if (isset($_GET['edit_id'])) {
    ...
    $edit_query = "SELECT * FROM tabletacc WHERE id='$edit_id'";
...
<head>
    <meta charset="UTF-8">
    <title>Edit Tablet Accessory Product</title>
    <style>
        body {
...
<body>
<?php if ($edit_data) { ?>
    <h2>Edit Tablet Accessory Product</h2>
    <form method="POST" enctype="multipart/form-data">
        ...
        <label>Tablet Accessory Brand:</label>
        <select name="processor_dropdown" required>
            ...
            while ($Brand = mysqli_fetch_assoc($brand_result)) { ?>
                <option value="<?= htmlspecialchars($Brand['tabletaccbrand']); ?>"
                    <?= ($edit_data['processor'] == $Brand['tabletaccbrand']) ? 'selected' : ''; ?>>
                    <?= htmlspecialchars($Brand['tabletaccbrand']); ?>
                </option>
            <?php } ?>
        </select>
        ...
        <select name="store_dropdown" required>
            ...
            while ($Store = mysqli_fetch_assoc($store_result)) { ?>
                <option value="<?= htmlspecialchars($Store['tabletaccstore']); ?>"
                    <?= ($edit_data['brand'] == $Store['tabletaccstore']) ? 'selected' : ''; ?>>
                    <?= htmlspecialchars($Store['tabletaccstore']); ?>
                </option>
            <?php } ?>
        </select>
        ...
        <img src="images/<?= htmlspecialchars($edit_data['image']); ?>" alt="Tablet Accessory Image">
...
<?php } else { ?>
    <p class="error">Tablet accessory product not found. Please provide a valid `edit_id` in the URL.</p>
<?php } ?>
</body>
</html>
