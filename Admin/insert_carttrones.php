...
if (isset($_POST['btn'])) {
    ...
    $brand = mysqli_real_escape_string($conn, $_POST['brand_dropdown']); // from carttrones_store
    $processor = mysqli_real_escape_string($conn, $_POST['processor_dropdown']); // from carttrones_brand
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($temp, $final)) {
            ...
            $insert_query = "INSERT INTO carttrones 
            ...
            if (mysqli_query($conn, $insert_query)) {
                echo "<p style='color: green;'>Cart Trones inserted successfully.</p>";
            } else {
...
$brand_result = mysqli_query($conn, "SELECT * FROM carttrones_store");
$processor_result = mysqli_query($conn, "SELECT * FROM carttrones_brand");
?>
...
<body>
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <label for="brand_dropdown">Select Brand (from carttrones_store)</label>
        <select name="brand_dropdown" required>
            <option value="" disabled selected>Select a brand</option>
            <?php while ($row = mysqli_fetch_assoc($brand_result)) {
                $val = htmlspecialchars($row['cattronesstore']);
                echo "<option value='$val'>$val</option>";
            } ?>
        </select>

        <label for="processor_dropdown">Select Processor (from carttrones_brand)</label>
        <select name="processor_dropdown" required>
            ...
            <?php while ($row = mysqli_fetch_assoc($processor_result)) {
                $val = htmlspecialchars($row['carttronesbrand']);
                echo "<option value='$val'>$val</option>";
            } ?>
        </select>
