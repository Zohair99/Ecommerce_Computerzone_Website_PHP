...
$graphic_query = "SELECT * FROM graphic_card";
...
if (isset($_POST['update_product']) && isset($_POST['update_id'])) {
    ...
    $gpu_name = mysqli_real_escape_string($conn, $_POST['graphic_dropdown']); // Use gpu_name instead of ID
    $chip_set = mysqli_real_escape_string($conn, $_POST['chip_dropdown']); // Use chip_set instead of ID
    ...
    if (mysqli_query($conn, $update_query)) {
        echo "<script>alert('Product updated successfully.'); window.location.href='view_gpu.php';</script>";
    } else {
...
if (isset($_GET['edit_id'])) {
    ...
    $edit_query = "SELECT * FROM gpu WHERE id='$edit_id'";
...
<?php if ($edit_data) { ?>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <select name="graphic_dropdown" required>
            <option value="">Select GPU</option>
            <?php
            mysqli_data_seek($graphic_result, 0);
            while ($gpu = mysqli_fetch_assoc($graphic_result)) { ?>
                <option value="<?php echo $gpu['gpu_name']; ?>" 
                    <?php if (isset($edit_data['gpu_name']) && $edit_data['gpu_name'] == $gpu['gpu_name']) echo 'selected'; ?>>
        ...
        <label>Chipset:</label>
        <select name="chip_dropdown" required>
            <option value="">Select Chipset</option>
            <?php
            mysqli_data_seek($chipset_result, 0);
            while ($chipset = mysqli_fetch_assoc($chipset_result)) { ?>
                <option value="<?php echo $chipset['chip_set']; ?>" 
                    <?php if (isset($edit_data['chip_set']) && $edit_data['chip_set'] == $chipset['chip_set']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($chipset['chip_set']); ?>
                </option>
            <?php } ?>
        </select>
        ...
        <input type="text" name="codee" value="<?php echo htmlspecialchars($edit_data['codee']); ?>" required>
