...
// Fetch brands for the dropdown (from pos_brand)
$brand_query = "SELECT id, posbrand FROM pos_brand ORDER BY posbrand ASC";
$brand_result = mysqli_query($conn, $brand_query);

// Fetch categories/stores for the dropdown (from pos_store)
$store_query = "SELECT id, posstore FROM pos_store ORDER BY posstore ASC";
...
if (isset($_POST['update_product']) && isset($_POST['update_id'])) {
    ...
    $processor = mysqli_real_escape_string($conn, $_POST['processor_dropdown']); // This is now 'posbrand'
    $brand = mysqli_real_escape_string($conn, $_POST['store_dropdown']);       // This is now 'posstore'
    ...
    // Update query for the 'pos' table
    $update_query = "UPDATE pos SET
                     offer='$offer',
                     name='$name',
                     review='$review',
                     code='$codee',
                     ...
                     image='$file_name',
                     brand='$brand',      -- Corresponds to posstore
                     processor='$processor', -- Corresponds to posbrand
                     spec1='$spec1',
                     spec2='$spec2',
                     spec3='$spec3',
                     spec4='$spec4',
    ...
    if (mysqli_query($conn, $update_query)) {
        echo "<script>alert('Product updated successfully.'); window.location.href='view_pos.php';</script>"; // Redirect to your POS view page
    } else {
        die("SQL Error in Update: " . mysqli_error($conn));
    }
}

// Fetch product details for editing from 'pos' table
$edit_data = null;
if (isset($_GET['edit_id'])) {
    $edit_id = intval($_GET['edit_id']);
    $edit_query = "SELECT * FROM pos WHERE id='$edit_id'";
...
<body>
    <?php if ($edit_data) { ?>
        <h2>Edit Point of Sale (POS) Product</h2>
        <form method="POST" enctype="multipart/form-data">
            ...
            <select name="processor_dropdown" required>
                ...
                <?php
                // Reset the pointer of the brand result set (pos_brand)
                mysqli_data_seek($brand_result, 0);
                while ($Brand = mysqli_fetch_assoc($brand_result)) { ?>
                    <option value="<?php echo htmlspecialchars($Brand['posbrand']); ?>"
                        <?php if ($edit_data['processor'] == $Brand['posbrand']) echo 'selected'; ?>>
                        <?php echo htmlspecialchars($Brand['posbrand']); ?>
                    </option>
                <?php } ?>
            </select>
            ...
            <select name="store_dropdown" required>
                ...
                <?php
                // Reset the pointer of the store result set (pos_store)
                mysqli_data_seek($store_result, 0);
                while ($Store = mysqli_fetch_assoc($store_result)) { ?>
                    <option value="<?php echo htmlspecialchars($Store['posstore']); ?>"
                        <?php if ($edit_data['brand'] == $Store['posstore']) echo 'selected'; ?>>
                        <?php echo htmlspecialchars($Store['posstore']); ?>
                    </option>
                <?php } ?>
            </select>
