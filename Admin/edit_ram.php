...
// Fetch memory and capacity for the dropdowns
$memory_query = "SELECT * FROM memory";
$memory_result = mysqli_query($conn, $memory_query);

$capacity_query = "SELECT * FROM r_capacity";
...
if (isset($_POST['update_product']) && isset($_POST['update_id'])) {
    ...
    $memory = mysqli_real_escape_string($conn, $_POST['memory_dropdown']); // Use memory module
    $capacity = mysqli_real_escape_string($conn, $_POST['capacity_dropdown']); // Use capacity
    ...
    if (mysqli_query($conn, $update_query)) {
        echo "<script>alert('Product updated successfully.'); window.location.href='view_ram.php';</script>";
    } else {
...
if (isset($_GET['edit_id'])) {
    ...
    $edit_query = "SELECT * FROM ram WHERE id='$edit_id'";
...
<?php if ($edit_data) { ?>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <select name="memory_dropdown" required>
            <option value="">Select Memory Module</option>
            <?php
            mysqli_data_seek($memory_result, 0);
            while ($memory = mysqli_fetch_assoc($memory_result)) { ?>
                <option value="<?php echo $memory['module']; ?>" 
                    <?php if (isset($edit_data['memory']) && $edit_data['memory'] == $memory['module']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($memory['module']); ?>
                </option>
            <?php } ?>
        </select>

        <label>Capacity:</label>
        <select name="capacity_dropdown" required>
            <option value="">Select Capacity</option>
            <?php
            mysqli_data_seek($capacity_result, 0);
            while ($capacity = mysqli_fetch_assoc($capacity_result)) { ?>
                <option value="<?php echo $capacity['capacity']; ?>" 
                    <?php if (isset($edit_data['store']) && $edit_data['store'] == $capacity['capacity']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($capacity['capacity']); ?>
                </option>
            <?php } ?>
        </select>
