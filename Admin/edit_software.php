...
// Fetch brands for the dropdown (from software_brand)
$brand_query = "SELECT id, softwarebrand FROM software_brand ORDER BY softwarebrand ASC";
$brand_result = mysqli_query($conn, $brand_query);

// Fetch stores for the dropdown (from software_store)
$store_query = "SELECT id, softwarestore FROM software_store ORDER BY softwarestore ASC";
...
if (isset($_POST['update_product']) && isset($_POST['update_id'])) {
    ...
    if (mysqli_query($conn, $update_query)) {
        echo "<script>alert('Software product updated successfully.'); window.location.href='view_software.php';</script>";
    } else {
...
if (isset($_GET['edit_id'])) {
    ...
    $edit_query = "SELECT * FROM software WHERE id='$edit_id'";
...
<head>
    <meta charset="UTF-8">
    <title>Edit Software Product</title>
    <style>
        body {
...
<?php if ($edit_data) { ?>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <label>Software Brand:</label>
        <select name="processor_dropdown" required>
            <option value="">Select Software Brand</option>
            <?php
            mysqli_data_seek($brand_result, 0);
            while ($Brand = mysqli_fetch_assoc($brand_result)) { ?>
                <option value="<?php echo htmlspecialchars($Brand['softwarebrand']); ?>"
                    <?php if ($edit_data['processor'] == $Brand['softwarebrand']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($Brand['softwarebrand']); ?>
                </option>
            <?php } ?>
        </select>
        ...
        <select name="store_dropdown" required>
            ...
            while ($Store = mysqli_fetch_assoc($store_result)) { ?>
                <option value="<?php echo htmlspecialchars($Store['softwarestore']); ?>"
                    <?php if ($edit_data['brand'] == $Store['softwarestore']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($Store['softwarestore']); ?>
                </option>
            <?php } ?>
        </select>
        ...
        <img src="images/<?php echo htmlspecialchars($edit_data['image']); ?>" alt="Software Image">
...
<?php } else { ?>
    <p class="error">Software product not found. Please provide a valid `edit_id` in the URL.</p>
<?php } ?>
</body>
</html>
