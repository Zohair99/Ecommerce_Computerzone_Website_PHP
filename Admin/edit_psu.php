...
// Fetch power and wattage for the dropdowns
...
if (isset($_POST['update_product']) && isset($_POST['update_id'])) {
    ...
    $power = mysqli_real_escape_string($conn, $_POST['power_dropdown']); // Use power supply
    $wattage = mysqli_real_escape_string($conn, $_POST['wattage_dropdown']); // Use wattage
    ...
    if (mysqli_query($conn, $update_query)) {
        echo "<script>alert('Product updated successfully.'); window.location.href='view_psu.php';</script>";
    } else {
...
if (isset($_GET['edit_id'])) {
    ...
    $edit_query = "SELECT * FROM psu WHERE id='$edit_id'";
...
<?php if ($edit_data) { ?>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <select name="power_dropdown" required>
            <option value="">Select Power Supply</option>
            <?php
            mysqli_data_seek($power_result, 0);
            while ($power = mysqli_fetch_assoc($power_result)) { ?>
                <option value="<?php echo $power['supply']; ?>"
                    <?php if (isset($edit_data['power']) && $edit_data['power'] == $power['supply']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($power['supply']); ?>
                </option>
            <?php } ?>
        </select>

        <label>Wattage:</label>
        <select name="wattage_dropdown" required>
            <option value="">Select Wattage</option>
            <?php
            mysqli_data_seek($wattage_result, 0);
            while ($wattage = mysqli_fetch_assoc($wattage_result)) { ?>
                <option value="<?php echo $wattage['watt']; ?>"
                    <?php if (isset($edit_data['wattage']) && $edit_data['wattage'] == $wattage['watt']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($wattage['watt']); ?>
                </option>
            <?php } ?>
        </select>
