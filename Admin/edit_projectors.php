...
// Fetch brands for the dropdown (from projectors_brand)
$brand_query = "SELECT id, projectorsbrand FROM projectors_brand ORDER BY projectorsbrand ASC";
$brand_result = mysqli_query($conn, $brand_query);

// Fetch categories/stores for the dropdown (from projectors_store)
$store_query = "SELECT id, projectorsstore FROM projectors_store ORDER BY projectorsstore ASC";
...
if (isset($_POST['update_product']) && isset($_POST['update_id'])) {
    ...
    $processor = mysqli_real_escape_string($conn, $_POST['processor_dropdown']); // projectorsbrand
    $brand = mysqli_real_escape_string($conn, $_POST['store_dropdown']);         // projectorsstore
    ...
    $update_query = "UPDATE projectors SET
                     offer='$offer',
                     name='$name',
                     review='$review',
                     code='$codee',
    ...
    if (mysqli_query($conn, $update_query)) {
        echo "<script>alert('Projector updated successfully.'); window.location.href='view_projectors.php';</script>";
    } else {
...
if (isset($_GET['edit_id'])) {
    ...
    $edit_query = "SELECT * FROM projectors WHERE id='$edit_id'";
...
<?php if ($edit_data) { ?>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <label>Projector Brand:</label>
        <select name="processor_dropdown" required>
            <option value="">Select Projector Brand</option>
            <?php
            mysqli_data_seek($brand_result, 0);
            while ($Brand = mysqli_fetch_assoc($brand_result)) { ?>
                <option value="<?php echo htmlspecialchars($Brand['projectorsbrand']); ?>"
                    <?php if ($edit_data['processor'] == $Brand['projectorsbrand']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($Brand['projectorsbrand']); ?>
                </option>
            <?php } ?>
        </select>
        ...
        <select name="store_dropdown" required>
            ...
            while ($Store = mysqli_fetch_assoc($store_result)) { ?>
                <option value="<?php echo htmlspecialchars($Store['projectorsstore']); ?>"
                    <?php if ($edit_data['brand'] == $Store['projectorsstore']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($Store['projectorsstore']); ?>
                </option>
            <?php } ?>
        </select>
...
<?php } else { ?>
    <p class="error">Projector not found. Please provide a valid `edit_id` in the URL.</p>
<?php } ?>
</body>
</html>
