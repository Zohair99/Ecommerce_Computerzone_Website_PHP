...
if (isset($_POST['btn'])) {
    ...
    // For 'brand' in processors table, we need the actual string value from processors_store
    $store_category = mysqli_real_escape_string($conn, $_POST['store_dropdown']);

    // For 'processor' in processors table, we need the actual string value from processors_brand
    $processor_brand = mysqli_real_escape_string($conn, $_POST['brand_dropdown']);
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($temp, $final)) {
            ...
            // Insert query into the 'processors' table
            $insert_query = "INSERT INTO processors (offer, name, review, code, description, image, brand, processor, spec1, spec2, spec3, spec4, price, old_price) 
                             VALUES ('$offer', '$name', '$review', '$code', '$description', '$file_name', '$store_category', '$processor_brand', '$spec1', '$spec2', '$spec3', '$spec4', '$price', '$old_price')";

            if (mysqli_query($conn, $insert_query)) {
                echo "<p style='color: green;'>Record inserted successfully into Processors.</p>";
            } else {
...
<head>
    ...
    <title>Insert Processor Data</title>
...
<body>
    <h2>Insert New Processor Product</h2>
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-outline mb-4">
            <select class="form-select" name="store_dropdown" id="store_dropdown" required>
                ...
                <?php
                // Display categories/stores from processors_store
                while ($row = mysqli_fetch_assoc($store_result)) {
                    $store_category_name = $row['processorsstore'];
        ...
        <label for="brand_dropdown">Select Processor Brand </label>
        <div class="form-outline mb-4">
            <select class="form-select" name="brand_dropdown" id="brand_dropdown" required>
                <option selected disabled>Select a Brand</option>
                <?php
                // Display brands from processors_brand
                while ($row = mysqli_fetch_assoc($brand_result)) {
                    $processor_brand_name = $row['processorsbrand'];
