...
// Fetch brands for the dropdown (from ups_brand)
$brand_query = "SELECT id, upsbrand FROM ups_brand ORDER BY upsbrand ASC";
$brand_result = mysqli_query($conn, $brand_query);

// Fetch stores for the dropdown (from ups_store)
$store_query = "SELECT id, upsstore FROM ups_store ORDER BY upsstore ASC";
...
if (isset($_POST['update_product']) && isset($_POST['update_id'])) {
    ...
    // 'processor' in ups table maps to 'upsbrand' (the brand of the UPS device)
    $processor = mysqli_real_escape_string($conn, $_POST['processor_dropdown']);
    // 'brand' in ups table maps to 'upsstore' (the category/store of the UPS device)
    ...
    // Update query - Changed table name to 'ups'
    $update_query = "UPDATE ups SET
                     offer='$offer',
                     name='$name',
                     review='$review',
                     code='$codee',
                     ...
                     image='$file_name',
                     brand='$brand',      -- Corresponds to upsstore
                     processor='$processor', -- Corresponds to upsbrand
                     spec1='$spec1',
                     spec2='$spec2',
                     spec3='$spec3',
                     spec4='$spec4',
    ...
    if (mysqli_query($conn, $update_query)) {
        ...
        echo "<script>alert('UPS product updated successfully.'); window.location.href='view_ups.php';</script>";
    } else {
...
if (isset($_GET['edit_id'])) {
    ...
    // Changed table name to 'ups'
    $edit_query = "SELECT * FROM ups WHERE id='$edit_id'";
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <select name="processor_dropdown" required>
            ...
            while ($Brand = mysqli_fetch_assoc($brand_result)) { ?>
                <option value="<?php echo htmlspecialchars($Brand['upsbrand']); ?>"
                    <?php if ($edit_data['processor'] == $Brand['upsbrand']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($Brand['upsbrand']); ?>
                </option>
            <?php } ?>
        </select>
        ...
        <select name="store_dropdown" required>
            ...
            while ($Store = mysqli_fetch_assoc($store_result)) { ?>
                <option value="<?php echo htmlspecialchars($Store['upsstore']); ?>"
                    <?php if ($edit_data['brand'] == $Store['upsstore']) echo 'selected'; ?>>
                    <?php echo htmlspecialchars($Store['upsstore']); ?>
                </option>
            <?php } ?>
        </select>
        ...
        <img src="images/<?php echo htmlspecialchars($edit_data['image']); ?>" alt="UPS Image">
        ...
        <button type="submit" name="update_product">Update UPS Product</button>
    </form>
<?php } else { ?>
    <p class="error">UPS product not found. Please provide a valid `edit_id` in the URL.</p>
<?php } ?>
</body>
</html>