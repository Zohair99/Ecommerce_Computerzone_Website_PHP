...
if (isset($_POST['btn'])) {
    ...
    $memory = mysqli_real_escape_string($conn, $_POST['memory_dropdown']); // Updated to 'memory_dropdown'
    $capacity = mysqli_real_escape_string($conn, $_POST['capacity_dropdown']); // Updated to 'capacity_dropdown'
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($temp, $final)) {
            ...
            // Insert query (updated for ram table)
            $insert_query = "INSERT INTO ram (offer, name, review, code, description, image, memory, store, spec1, spec2, spec3, spec4, price, old_price) 
                             VALUES ('$offer', '$name', '$review', '$code', '$description', '$file_name', '$memory', '$capacity', '$spec1', '$spec2', '$spec3', '$spec4', '$price', '$old_price')";
...
$capacity_query = "SELECT * FROM r_capacity"; // Fetch data from 'r_capacity' table
...
<body>
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <label for="memory_dropdown">Select a Memory Module</label> <!-- Updated to 'memory_dropdown' -->
        <select class="form-select" name="memory_dropdown" id="memory_dropdown" required>
            <option selected disabled>Select a memory module</option>
            <?php
        ...
        <label for="capacity_dropdown">Select a Capacity</label> <!-- Updated to 'capacity_dropdown' -->
        <select class="form-select" name="capacity_dropdown" id="capacity_dropdown" required>
            <option selected disabled>Select a capacity</option>
            <?php
