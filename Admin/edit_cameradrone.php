...
$camdrone_query = "SELECT * FROM camdrone_store";
$camdrone_result = mysqli_query($conn, $camdrone_query);

$cambrand_query = "SELECT * FROM cambrand_store";
...
if (isset($_POST['update_product']) && isset($_POST['update_id'])) {
    ...
    $update_query = "UPDATE cameradrone SET
                    offer='$offer',
                    name='$name',
                    review='$review',
                    code='$code',
    ...
    if (mysqli_query($conn, $update_query)) {
        echo "<script>alert('Camera Drone record updated successfully.'); window.location.href='view_cameradrone.php';</script>";
    } else {
...
if (isset($_GET['edit_id'])) {
    ...
    $edit_query = "SELECT * FROM cameradrone WHERE id='$edit_id'";
...
<?php if ($edit_data) { ?>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <label>Drone (Camdrone):</label>
        <select name="brand_dropdown" required>
            <option value="">Select Drone</option>
            <?php
            mysqli_data_seek($camdrone_result, 0);
            while ($camdrone_row = mysqli_fetch_assoc($camdrone_result)) {
                $val = $camdrone_row['camdrone'];
        ...
        <label>Brand (Cambrand):</label>
        <select name="processor_dropdown" required>
            <option value="">Select Brand</option>
            <?php
            mysqli_data_seek($cambrand_result, 0);
            while ($cambrand_row = mysqli_fetch_assoc($cambrand_result)) {
                $val = $cambrand_row['cambrand'];
...
<?php } else { ?>
    <p style="color:red; text-align:center;">Camera drone not found.</p>
<?php } ?>
</body>
</html>
