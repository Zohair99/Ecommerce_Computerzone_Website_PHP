...
$title = 'Software Prices in Pakistan';
...
$sort_order = "software.price ASC";
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "software.price DESC";
            break;
        case "low":
            $sort_order = "software.price ASC";
            break;
        case "rating":
            $sort_order = "software.review DESC";
            break;
        default:
            $sort_order = "software.price ASC";
    }
...
if (!empty($selected_processor)) {
    $where_clauses[] = "software_brand.softwarebrand = ?";
    $params[] = $selected_processor;
    $types .= 's';
}

if (!empty($selected_brand)) {
    $where_clauses[] = "software_store.softwarestore = ?";
    $params[] = $selected_brand;
    $types .= 's';
}

if (!empty($searchQuery)) {
    $where_clauses[] = "software.name LIKE ?";
    $params[] = '%' . $searchQuery . '%';
    $types .= 's';
}
...
$total_products_query = "SELECT COUNT(*) as total
    FROM software
    INNER JOIN software_brand ON software.processor = software_brand.softwarebrand
    INNER JOIN software_store ON software.brand = software_store.softwarestore"
    . $where_clause_string;
...
$product_query = "SELECT software.*,
    software_brand.softwarebrand AS processor_name,
    software_store.softwarestore AS brand_name
    FROM software
    INNER JOIN software_brand ON software.processor = software_brand.softwarebrand
    INNER JOIN software_store ON software.brand = software_store.softwarestore"
    . $where_clause_string
...
<body>
    ...
    <nav class="breadcrumb">
        <ul>
            ...
            <li>/</li>
            <li><a href="software.php" class="active">Softwares</a></li>
        </ul>
    </nav>
        ...
        <aside class="sidebar">
            <div class="filter-category">
                ...
                <ul class="filter-list" style="display: none;">
                    <?php
                    // Query for software store categories
                    $store_query = "SELECT DISTINCT softwarestore FROM software_store ORDER BY softwarestore ASC";
                    ...
                    if (!$store_result) {
                        ...
                        while ($row = mysqli_fetch_assoc($store_result)) {
                            $original_value = $row['softwarestore'];
            ...
            <div class="filter-category">
                ...
                <ul class="filter-list" style="display: none;">
                    <?php
                    // Query for software brands
                    $brand_query = "SELECT DISTINCT softwarebrand FROM software_brand ORDER BY softwarebrand ASC";
                    ...
                    if (!$brand_result) {
                        ...
                        while ($row = mysqli_fetch_assoc($brand_result)) {
                            $original_value = $row['softwarebrand'];
                            $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                            $url_value = urlencode($original_value);
        ...
        <div class="main-content">
            <div class="product-grid">
                <div class="mainheading">
                    <h3 id="pos-heading">Softwares<?php echo $selected_brand ? " >> " . htmlspecialchars($selected_brand) : ''; ?></h3>
                </div>
                <div class="pp">
                    <p>Shop a wide selection of Softwares in Pakistan including various models at Czone.com.pk</p>
                </div>
                ...
                <?php while ($row = mysqli_fetch_assoc($product_result)): ?>
                    <div class="product-container card" data-category="projectors" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                        ...
                        <div class="product-details">
                            ...
                            <p class="category"><strong>Category:</strong> Software > <?php echo htmlspecialchars($row['brand_name']); ?></p>
                        ...
                        <div class="product-right">
                            ...
                            <?php endif; ?>
                            <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'software')">
            ...
            <div class="endpara">
                <p>Explore high-quality software solutions designed to meet diverse needs — from personal productivity and creative design to enterprise-level management and cybersecurity. Discover the right tools to elevate your digital experience.</p>
                <p>Whether you're creating, collaborating, or protecting your data, our selection of software offers powerful features, seamless performance, and great value. Shop now for the latest and most trusted software applications.</p>
            </div>
