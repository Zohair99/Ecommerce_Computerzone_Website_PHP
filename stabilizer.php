...
$title = 'Stabilizers Prices in Pakistan';
...
$sort_order = "stabilizer.price ASC";
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "stabilizer.price DESC";
            break;
        case "low":
            $sort_order = "stabilizer.price ASC";
            break;
        case "rating":
            $sort_order = "stabilizer.review DESC";
            break;
        default:
            $sort_order = "stabilizer.price ASC";
    }
...
if (!empty($selected_processor)) {
    $where_clauses[] = "stabilizer_brand.stabilizerbrand = ?";
    $params[] = $selected_processor;
    $types .= 's';
}

if (!empty($selected_brand)) {
    $where_clauses[] = "stabilizer_store.stabilizerstore = ?";
    $params[] = $selected_brand;
    $types .= 's';
}

if (!empty($searchQuery)) {
    $where_clauses[] = "stabilizer.name LIKE ?";
    $params[] = '%' . $searchQuery . '%';
    $types .= 's';
}
...
$total_products_query = "SELECT COUNT(*) as total
    FROM stabilizer
    INNER JOIN stabilizer_brand ON stabilizer.processor = stabilizer_brand.stabilizerbrand
    INNER JOIN stabilizer_store ON stabilizer.brand = stabilizer_store.stabilizerstore"
    . $where_clause_string;
...
$product_query = "SELECT stabilizer.*,
    stabilizer_brand.stabilizerbrand AS processor_name,
    stabilizer_store.stabilizerstore AS brand_name
    FROM stabilizer
    INNER JOIN stabilizer_brand ON stabilizer.processor = stabilizer_brand.stabilizerbrand
    INNER JOIN stabilizer_store ON stabilizer.brand = stabilizer_store.stabilizerstore"
    . $where_clause_string
...
<body>
    ...
    <nav class="breadcrumb">
        <ul>
            ...
            <li>/</li>
            <li><a href="stabilizer.php" class="active">Stabilizer</a></li>
        </ul>
    </nav>
    ...
    <div class="ppcontainer">
        <aside class="sidebar">
            <div class="filter-category">
                <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Stabilizers</button>
                <ul class="filter-list" style="display: none;">
                    <?php
                    $store_query = "SELECT DISTINCT stabilizerstore FROM stabilizer_store ORDER BY stabilizerstore ASC";
                    ...
                    if (!$store_result) {
                        ...
                        while ($row = mysqli_fetch_assoc($store_result)) {
                            $original_value = $row['stabilizerstore'];
            ...
            <div class="filter-category">
                ...
                <ul class="filter-list" style="display: none;">
                    <?php
                    $brand_query = "SELECT DISTINCT stabilizerbrand FROM stabilizer_brand ORDER BY stabilizerbrand ASC";
                    ...
                    if (!$brand_result) {
                        ...
                        while ($row = mysqli_fetch_assoc($brand_result)) {
                            $original_value = $row['stabilizerbrand'];
                            $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                            $url_value = urlencode($original_value);
        ...
        <div class="main-content">
            <div class="product-grid">
                <div class="mainheading">
                    <h3 id="pos-heading">Stabilizer<?php echo $selected_brand ? " >> " . htmlspecialchars($selected_brand) : ''; ?></h3>
                </div>
                <div class="pp">
                    <p>Shop a wide selection of Stabilizer in Pakistan including various models at Czone.com.pk</p>
                </div>
                ...
                <?php while ($row = mysqli_fetch_assoc($product_result)): ?>
                    <div class="product-container card" data-category="projectors" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                        ...
                        <div class="product-right">
                            ...
                            <?php endif; ?>
                            <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'stabilizer')">
            ...
            <div class="endpara">
                <p>Ensure the safety and longevity of your electronics with our premium range of voltage stabilizers. Whether you're protecting home appliances, entertainment systems, or critical office equipment, find the ideal stabilizer to maintain consistent power and prevent damage from voltage fluctuations.</p>
                <p>From compact units for single devices to robust solutions for entire households, our collection combines reliability with advanced technology. Discover stabilizers that deliver steady performance and peace of mind. Shop now and safeguard your valuable electronics.</p>
            </div>
        </div>
    </div>
