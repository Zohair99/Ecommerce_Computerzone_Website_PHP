...
function getRedirectUrl($baseUrl, $currentParams)
{
    ...
    if (!empty($currentParams['power'])) {
        $params['power'] = $currentParams['power'];
    }
    if (!empty($currentParams['wattage'])) {
        $params['wattage'] = $currentParams['wattage'];
    }
...
$title = 'Power Supply Prices in Pakistan - PSU Prices';
...
$sort_order = "psu.price ASC";

if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "psu.price DESC";
            break;
        case "low":
            $sort_order = "psu.price ASC";
            break;
        case "rating":
            $sort_order = "psu.review DESC";
            break;
        default:
            $sort_order = "psu.price ASC";
    }
...
$selected_power = isset($_GET['power']) ? $_GET['power'] : '';
$selected_wattage = isset($_GET['wattage']) ? urldecode($_GET['wattage']) : '';

$where_clauses = [];
$params = [];
$types = '';

if (!empty($selected_power)) {
    $where_clauses[] = "power.supply = ?";
    $params[] = $selected_power;
    $types .= 's';
}

if (!empty($selected_wattage)) {
    $where_clauses[] = "wattage.watt = ?";
    $params[] = $selected_wattage;
    $types .= 's';
}

if (!empty($searchQuery)) {
    $where_clauses[] = "psu.name LIKE ?";
    $params[] = '%' . $searchQuery . '%';
    $types .= 's';
}
...
$total_products_query = "SELECT COUNT(*) as total
                         FROM psu
                         INNER JOIN power ON psu.power = power.supply
                         INNER JOIN wattage ON psu.wattage = wattage.watt"
    . $where_clause_string;
...
$product_query = "SELECT psu.*, power.supply AS power_supply_name, wattage.watt AS wattage_name
                  FROM psu
                  INNER JOIN power ON psu.power = power.supply
                  INNER JOIN wattage ON psu.wattage = wattage.watt"
    . $where_clause_string
...
<nav class="breadcrumb">
    <ul>
        ...
        <li>/</li>
        <li><a href="psu.php" class="active">Power Supply Unit<?php echo $selected_power ? " >> " . htmlspecialchars($selected_power) : ''; ?></a></li>
    </ul>
</nav>
...
<div class="ppcontainer">
    ...
    <aside class="sidebar">
        ...
        <div class="filter-category">
            <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Power Supply</button>
            <ul class="filter-list" style="display: none;">
                <?php
                $supply_query = "SELECT DISTINCT supply FROM power ORDER BY supply ASC";
                ...
                if (!$supply_result) {
                    ...
                    while ($row = mysqli_fetch_assoc($supply_result)) {
                        $original_value = $row['supply'];
                        $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                        $url_value = urlencode($original_value);
                        $is_active = (isset($_GET['power']) && urldecode($_GET['power']) === $original_value) ? 'active' : '';
                        echo "<li class='$is_active'><a href='?power=$url_value'>$display_value</a></li>";
                    }
                }
                ?>
            </ul>
        ...
        <!-- Wattage Filter -->
        <div class="filter-category">
            <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Wattage</button>
            <ul class="filter-list" style="display: none;">
                <?php
                $wattage_query = "SELECT DISTINCT watt FROM wattage ORDER BY created_at DESC";
                $wattage_result = mysqli_query($conn, $wattage_query);

                if (!$wattage_result) {
                    echo "<li>Error fetching wattage options: " . htmlspecialchars(mysqli_error($conn)) . "</li>";
                } else {
                    while ($row = mysqli_fetch_assoc($wattage_result)) {
                        $original_value = $row['watt'];
                        $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                        $url_value = urlencode($original_value);
                        $is_active = (isset($_GET['wattage']) && urldecode($_GET['wattage']) === $original_value) ? 'active' : '';
                        echo "<li class='$is_active'><a href='?wattage=$url_value'>$display_value</a></li>";
                    }
                }
                ?>
            </ul>
    ...
    <div class="main-content">
        <div class="product-grid">
            <div class="mainheading">
                <h3 id="psu-heading">Power Supply Unit<?php echo $selected_power ? " >> " . htmlspecialchars($selected_power) : ''; ?></h3>
            </div>
            <div class="pp">
                <p>Shop a wide selection of Power Supply Units in Pakistan including various models at Czone.com.pk</p>
            </div>
            ...
            <?php while ($row = mysqli_fetch_assoc($product_result)): {
                    $power = isset($row['power']) ? htmlspecialchars($row['power']) : 'N/A';
                    $wattage = isset($row['wattage']) ? htmlspecialchars($row['wattage']) : 'N/A';
                } ?>
                <div class="product-container card" data-category="psu" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                    ...
                    <div class="product-details">
                        ...
                        <p class="category"><strong>Category:</strong> Power Supply > <?php echo $power; ?></p>
                        <p class="category"><strong>Wattage:</strong> <?php echo $wattage; ?></p>
                    ...
                    <div class="product-right">
                        ...
                        <?php endif; ?>
                        <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'psu')">
        ...
        <!-- End Paragraph -->
        <div class="endpara">
            <p>The Power Supply Unit (PSU) is a critical component of any computer or laptop, ensuring stable power delivery to the system. A reliable PSU prevents power surges and fluctuations that can damage sensitive hardware.</p>
            <p>We offer high-quality PSUs from top brands like Corsair, EVGA, and Thermaltake to ensure your system gets the power it needs, all at competitive prices in Pakistan.</p>
            <p>Enjoy long-lasting performance and great value with our selection of power supply units for every need and budget.</p>
        </div>
    </div>
</div>

<!-- Product Popup -->
<div id="productPopup" class="popup" data-category="psu">
    <div class="popup-content">
        ...
        <div class="aside">
            ...
            <button class="add-to-cart" onclick="addToCart(0, 'psu')">
                <i class="fa-solid fa-cart-shopping"></i> Add To Cart
            </button>
