...
$title = 'Desktop Computer Prices in Pakistan -Deskotp Computer Prices - Dell HP...';
...
$sort_order = "deskcomp.price ASC";
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "deskcomp.price DESC";
            break;
        case "low":
            $sort_order = "deskcomp.price ASC";
            break;
        case "rating":
            $sort_order = "deskcomp.review DESC";
            break;
    }
}
...
$base_product_query = "SELECT deskcomp.*, deskcomp_store.deskcompstore AS brand_name, deskcomp_brand.deskcompbrand AS processor_name
                       FROM deskcomp
                       INNER JOIN deskcomp_store ON deskcomp.brand = deskcomp_store.deskcompstore
                       INNER JOIN deskcomp_brand ON deskcomp.processor = deskcomp_brand.deskcompbrand";
...
if (!empty($selected_brand)) {
    $conditions[] = "deskcomp_store.deskcompstore = '" . $selected_brand . "'";
}
if (!empty($selected_processor)) {
    $conditions[] = "deskcomp_brand.deskcompbrand = '" . $selected_processor . "'";
}
if (!empty($searchQuery)) {
    $conditions[] = "deskcomp.name LIKE '%" . $searchQuery . "%'";
}
...
?>
    ...
    <nav class="breadcrumb">
        <ul>
            ...
            <li>/</li>
            <li><a href="desktopcomputers.php" class="active">Desktop Computers</a></li>
        </ul>
    </nav>
    ...
    <div class="ppcontainer">
        <!-- Left Sidebar -->
        <aside class="sidebar">
            <!-- Desktop Computers Filter -->
            <div class="filter-category">
                <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Desktop Computers</button>
                <ul class="filter-list" style="display: none;">
                    <?php
                    $store_query = "SELECT DISTINCT deskcompstore FROM deskcomp_store ORDER BY deskcompstore ASC";
                    ...
                    if (!$store_result) {
                        ...
                        while ($row = mysqli_fetch_assoc($store_result)) {
                            $original_value = $row['deskcompstore'];
            ...
            <div class="filter-category">
                ...
                <ul class="filter-list" style="display: none;">
                    <?php
                    $brand_query = "SELECT DISTINCT deskcompbrand FROM deskcomp_brand ORDER BY created_at DESC";
                    ...
                    if (!$brand_result) {
                        ...
                        while ($row = mysqli_fetch_assoc($brand_result)) {
                            $original_value = $row['deskcompbrand'];
                            $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                            $url_value = urlencode($original_value);
        ...
        <div class="main-content">
            <div class="product-grid">
                <div class="mainheading">
                    <h3 id="desktop-heading">Desktop Computers<?php echo $selected_brand ? " >> " . htmlspecialchars($selected_brand) : ''; ?></h3>
                </div>
                <div class="pp">
                    <p>Shop a wide selection of Desktop Computers in Pakistan including various models at Czone.com.pk</p>
                </div>
                ...
                <?php while ($row = mysqli_fetch_assoc($product_result)): {
                    ...
                    <div class="product-container card" data-category="desktop-computers" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                        ...
                        <div class="product-details">
                            ...
                            <p class="category"><strong>Category:</strong> Desktop Computers > <?php echo $brand; ?></p>
                        ...
                        <div class="product-right">
                            ...
                            <?php } ?>
                            <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'deskcomp')">
            ...
            <!-- End Paragraph -->
            <div class="endpara">
                <p>Get consistent, high-quality performance every time with our wide range of Desktop Computers.</p>
                <p>From everyday computing to high-performance workstations, we've got you covered with trusted brands like Dell, HP, and Lenovo.</p>
                <p>Enjoy powerful performance and great value, all at competitive prices in Pakistan.</p>
            </div>
        </div>
    </div>

    <!-- Product Popup -->
    <div id="productPopup" class="popup" data-category="desktop-computers">
        <div class="popup-content">
            ...
            <div class="aside">
                ...
                <button class="add-to-cart" onclick="addToCart(0, 'deskcomp')">
                    <i class="fa-solid fa-cart-shopping"></i> Add To Cart
                </button>
