...
$title = 'Affordable POS Barcode scanner and Recipt Printer Prices in Pakistan';
...
$sort_order = "pos.price ASC"; // Changed table alias to 'pos'

if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "pos.price DESC"; // Changed table alias to 'pos'
            break;
        case "low":
            $sort_order = "pos.price ASC"; // Changed table alias to 'pos'
            break;
        case "rating":
            $sort_order = "pos.review DESC"; // Changed table alias to 'pos'
            break;
        default:
            $sort_order = "pos.price ASC"; // Changed table alias to 'pos'
    }
...
if (!empty($selected_processor)) {
    $where_clauses[] = "pos_brand.posbrand = ?";
    $params[] = $selected_processor;
    $types .= 's';
}

if (!empty($selected_brand)) {
    $where_clauses[] = "pos_store.posstore = ?";
    $params[] = $selected_brand;
    $types .= 's';
}

if (!empty($searchQuery)) {
    $where_clauses[] = "pos.name LIKE ?";
    $params[] = '%' . $searchQuery . '%';
    $types .= 's';
}
...
$total_products_query = "SELECT COUNT(*) as total
                         FROM pos
                         INNER JOIN pos_brand ON pos.processor = pos_brand.posbrand
                         INNER JOIN pos_store ON pos.brand = pos_store.posstore"
    . $where_clause_string;
...
$product_query = "SELECT pos.*,
                    pos_brand.posbrand AS processor_name,
                    pos_store.posstore AS brand_name
                  FROM pos
                  INNER JOIN pos_brand ON pos.processor = pos_brand.posbrand
                  INNER JOIN pos_store ON pos.brand = pos_store.posstore"
    . $where_clause_string
...
<body>
    ...
    <nav class="breadcrumb">
        <ul>
            ...
            <li>/</li>
            <li><a href="pointofsales-pos.php" class="active">Point Of Sale POS</a></li>
        </ul>
    </nav>
    ...
    <div class="ppcontainer">
        <aside class="sidebar">
            <div class="filter-category">
                <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Point Of Sale POS</button>
                <ul class="filter-list" style="display: none;">
                    <?php
                    // Query for POS categories/stores
                    $store_query = "SELECT DISTINCT posstore FROM pos_store ORDER BY posstore ASC";
            ...
            <div class="filter-category">
                ...
                <ul class="filter-list" style="display: none;">
                    <?php
                    // Query for brands of POS
                    $brand_query = "SELECT DISTINCT posbrand FROM pos_brand ORDER BY posbrand ASC";
                    ...
                    if (!$brand_result) {
                        ...
                        while ($row = mysqli_fetch_assoc($brand_result)) {
                            $original_value = $row['posbrand'];
                            $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                            $url_value = urlencode($original_value);
                            // Using 'processor' GET parameter to filter by brand, as per your PHP logic
        ...
        <div class="main-content">
            <div class="product-grid">
                <div class="mainheading">
                    <h3 id="pos-heading">Point Of Sale POS<?php echo $selected_processor ? " >> " . htmlspecialchars($selected_processor) : ''; ?></h3>
                </div>
                <div class="pp">
                    <p>Shop a wide selection of Point Of Sale POS in Pakistan including various models at Czone.com.pk</p>
                </div>
                ...
                <?php while ($row = mysqli_fetch_assoc($product_result)): ?>
                    <div class="product-container card" data-category="pos" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                        ...
                        <div class="product-details">
                            ...
                            <p class="category"><strong>Category:</strong> Point Of Sale POS > <?php echo htmlspecialchars($row['brand_name']); ?></p>
                        ...
                        <div class="product-right">
                            ...
                            <?php endif; ?>
                            <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'pos')">
            ...
            <div class="endpara">
                <p>Looking for reliable and affordable POS system accessories in Pakistan? Whether you need barcode scanners, thermal receipt printers, cash drawers, or thermal
                    paper rolls, we offer competitive prices on all essential POS equipment. Perfect for retail stores, restaurants, pharmacies, and supermarkets, our accessories
                    are sourced from trusted brands to ensure smooth business operations. Compare prices, features, and order online with fast delivery anywhere in Pakistan.
                </p>
            </div>
        </div>
    </div>
