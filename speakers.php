...
$title = 'Speakers & Woofers Prices in Pakistan';
...
// Changed 'software.price' and 'software.review' to 'speaker.price' and 'speaker.review'
$sort_order = "speaker.price ASC";
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "speaker.price DESC";
            break;
        case "low":
            $sort_order = "speaker.price ASC";
            break;
        case "rating":
            $sort_order = "speaker.review DESC";
            break;
        default:
            $sort_order = "speaker.price ASC";
    }
...
// Changed 'software_brand.softwarebrand' to 'speaker_brand.speakerbrand'
if (!empty($selected_processor)) {
    $where_clauses[] = "speaker_brand.speakerbrand = ?";
    $params[] = $selected_processor;
    $types .= 's';
}

// Changed 'software_store.softwarestore' to 'speaker_store.speakerstore'
if (!empty($selected_brand)) {
    $where_clauses[] = "speaker_store.speakerstore = ?";
    $params[] = $selected_brand;
    $types .= 's';
}

// Changed 'software.name' to 'speaker.name'
if (!empty($searchQuery)) {
    $where_clauses[] = "speaker.name LIKE ?";
    $params[] = '%' . $searchQuery . '%';
    $types .= 's';
}
...
// Total count query - changed table names and join conditions
$total_products_query = "SELECT COUNT(*) as total
    FROM speaker
    INNER JOIN speaker_brand ON speaker.processor = speaker_brand.speakerbrand
    INNER JOIN speaker_store ON speaker.brand = speaker_store.speakerstore"
    . $where_clause_string;
...
// Fetch paginated products - changed table names and join conditions
$product_query = "SELECT speaker.*,
    speaker_brand.speakerbrand AS processor_name,
    speaker_store.speakerstore AS brand_name
    FROM speaker
    INNER JOIN speaker_brand ON speaker.processor = speaker_brand.speakerbrand
    INNER JOIN speaker_store ON speaker.brand = speaker_store.speakerstore"
    . $where_clause_string
...
<body>
    ...
    <div class="filter-category">
        <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Speakers</button>
        <ul class="filter-list" style="display: none;">
            <?php
            $store_query = "SELECT DISTINCT speakerstore FROM speaker_store ORDER BY speakerstore ASC";
            ...
            if (!$store_result) {
                ...
                while ($row = mysqli_fetch_assoc($store_result)) {
                    $original_value = $row['speakerstore'];
    ...
    <div class="filter-category">
        ...
        <ul class="filter-list" style="display: none;">
            <?php
            $brand_query = "SELECT DISTINCT speakerbrand FROM speaker_brand ORDER BY speakerbrand ASC";
            ...
            if (!$brand_result) {
                ...
                while ($row = mysqli_fetch_assoc($brand_result)) {
                    $original_value = $row['speakerbrand'];
                    $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                    $url_value = urlencode($original_value);
        ...
        <div class="main-content">
            <div class="product-grid">
                <div class="mainheading">
                    <h3 id="pos-heading">Speakers<?php echo $selected_brand ? " >> " . htmlspecialchars($selected_brand) : ''; ?></h3>
                </div>
                <div class="pp">
                    <p>Shop a wide selection of Speakers in Pakistan including various models at Czone.com.pk</p>
                </div>
                ...
                <?php while ($row = mysqli_fetch_assoc($product_result)): ?>
                    <div class="product-container card" data-category="projectors" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                        ...
                        <div class="product-details">
                            ...
                            <p class="category"><strong>Category:</strong> Speaker > <?php echo htmlspecialchars($row['brand_name']); ?></p>
                        ...
                        <div class="product-right">
                            ...
                            <?php endif; ?>
                            <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'speaker')">
            ...
            <div class="endpara">
                <p>Immerse yourself in crystal-clear audio with our premium selection of speakers. Whether you're seeking rich bass for your music, crisp dialogue for movies, or dynamic sound for gaming, find the perfect audio solution to elevate your listening experience.</p>
                <p>From portable Bluetooth speakers to powerful home theater systems, our collection offers unparalleled sound quality and cutting-edge technology. Discover speakers that bring every note, word, and effect to life. Shop now and hear the difference.</p>
            </div>
        </div>
    </div>
