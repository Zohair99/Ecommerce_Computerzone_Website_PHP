...
$title = 'Scanners Prices in Pakistan';
...
$sort_order = "scanners.price ASC";
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "scanners.price DESC";
            break;
        case "low":
            $sort_order = "scanners.price ASC";
            break;
        case "rating":
            $sort_order = "scanners.review DESC";
            break;
        default:
            $sort_order = "scanners.price ASC";
    }
...
if (!empty($selected_processor)) {
    $where_clauses[] = "scanners_brand.scannersbrand = ?";
    $params[] = $selected_processor;
    $types .= 's';
}

if (!empty($selected_brand)) {
    $where_clauses[] = "scanners_store.scannersstore = ?";
    $params[] = $selected_brand;
    $types .= 's';
}

if (!empty($searchQuery)) {
    $where_clauses[] = "scanners.name LIKE ?";
    $params[] = '%' . $searchQuery . '%';
    $types .= 's';
}
...
$total_products_query = "SELECT COUNT(*) as total
    FROM scanners
    INNER JOIN scanners_brand ON scanners.processor = scanners_brand.scannersbrand
    INNER JOIN scanners_store ON scanners.brand = scanners_store.scannersstore"
    . $where_clause_string;
...
$product_query = "SELECT scanners.*,
    scanners_brand.scannersbrand AS processor_name,
    scanners_store.scannersstore AS brand_name
    FROM scanners
    INNER JOIN scanners_brand ON scanners.processor = scanners_brand.scannersbrand
    INNER JOIN scanners_store ON scanners.brand = scanners_store.scannersstore"
    . $where_clause_string
...
<body>
    ...
    <nav class="breadcrumb">
        <ul>
            ...
            <li>/</li>
            <li><a href="scanner.php" class="active">Scanners</a></li>
        </ul>
    </nav>
        ...
        <aside class="sidebar">
            <div class="filter-category">
                ...
                <ul class="filter-list" style="display: none;">
                    <?php
                    // Query for scanner store categories
                    $store_query = "SELECT DISTINCT scannersstore FROM scanners_store ORDER BY scannersstore ASC";
                    ...
                    if (!$store_result) {
                        ...
                        while ($row = mysqli_fetch_assoc($store_result)) {
                            $original_value = $row['scannersstore'];
            ...
            <div class="filter-category">
                ...
                <ul class="filter-list" style="display: none;">
                    <?php
                    // Query for scanner brands
                    $brand_query = "SELECT DISTINCT scannersbrand FROM scanners_brand ORDER BY scannersbrand ASC";
                    ...
                    if (!$brand_result) {
                        ...
                        while ($row = mysqli_fetch_assoc($brand_result)) {
                            $original_value = $row['scannersbrand'];
                            $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                            $url_value = urlencode($original_value);
        ...
        <div class="main-content">
            <div class="product-grid">
                <div class="mainheading">
                    <h3 id="pos-heading">Scanners<?php echo $selected_brand ? " >> " . htmlspecialchars($selected_brand) : ''; ?></h3>
                </div>
                <div class="pp">
                    <p>Shop a wide selection of Scanners in Pakistan including various models at Czone.com.pk</p>
                </div>
                ...
                <?php while ($row = mysqli_fetch_assoc($product_result)): ?>
                    <div class="product-container card" data-category="projectors" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                        ...
                        <div class="product-details">
                            ...
                            <p class="category"><strong>Category:</strong> Scanners > <?php echo htmlspecialchars($row['brand_name']); ?></p>
                        ...
                        <div class="product-right">
                            ...
                            <?php endif; ?>
                            <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'scanners')">
            ...
            <div class="endpara">
                <p>Explore high-performance scanners designed for every need — from home office tasks and professional photography to business document management. Find the perfect scanning solution for your workflow.</p>
                <p>Whether you're digitizing photos, scanning important paperwork, or managing bulk documents, our range of scanners offers precision, speed, and excellent value. Shop now for the latest models.</p>
            </div>
