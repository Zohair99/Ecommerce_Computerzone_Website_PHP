...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    function getRedirectUrl($baseUrl) {
        ...
        if (!empty($_GET['brand'])) $params['brand'] = $_GET['brand'];
    ...
    if (isset($_POST['increase_quantity'])) {
        ...
        header("Location: " . getRedirectUrl($_SERVER['PHP_SELF']));
        exit();
    }
    ...
    if (isset($_POST['remove_item'])) {
        ...
        header("Location: " . getRedirectUrl('laptop.php'));
        exit();
...
$title = 'Laptop Prices in Pakistan | Laptops';
...
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high": $sort_order = "price DESC"; break;
        case "low": $sort_order = "price ASC"; break;
        case "rating": $sort_order = "review DESC"; break;
    }
}

$base_product_query = "SELECT p.*, b.Brand AS brand_name FROM product p JOIN brand b ON p.brand = b.id";
$base_count_query = "SELECT COUNT(*) as total FROM product p JOIN brand b ON p.brand = b.id";

$conditions = [];
if (!empty($selected_brand)) {
    $conditions[] = "b.Brand = '" . mysqli_real_escape_string($conn, $selected_brand) . "'";
}
if (!empty($searchQuery)) {
    $conditions[] = "p.name LIKE '%" . mysqli_real_escape_string($conn, $searchQuery) . "%'";
}
...
<body>
    ...
    <nav class="breadcrumb">
        <ul>
            ...
            <li>/</li>
            <li><a href="laptop.php" class="active">Laptops </a></li>
        </ul>
    </nav>
    ...
    <div class="ppcontainer">
        ...
        <aside class="sidebar">
            <div class="filter-category">
                <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Laptops</button>
                <ul class="filter-list" style="display: none;">
                    <?php
                    $brand_query = "SELECT * FROM brand";
                    $brand_result = mysqli_query($conn, $brand_query);
                    while ($row = mysqli_fetch_assoc($brand_result)) {
                        $brand_name = htmlspecialchars($row['Brand']);
                        echo "<li><a href='laptop.php?brand=" . urlencode($brand_name) . "'>$brand_name</a></li>";
                    }
                    ?>
                </ul>
            </div>
            ...
            <div class="filter-category">
                <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Warranty</button>
                <ul class="filter-list" style="display: none;">
                    <?php
                    $brand_query = "SELECT * FROM brand";
                    $brand_result = mysqli_query($conn, $brand_query);
        ...
        <div class="main-content">
            <div class="product-grid">
                <div class="mainheading">
                    <h3 id="laptop-heading">Laptops <?php echo $selected_brand ? ">> " . $selected_brand : ''; ?></h3>
                </div>
                <div class="pp">
                    <p>Shop a wide selection of Laptops in Pakistan at Czone.com.pk</p>
                </div>
                ...
                <?php while ($row = mysqli_fetch_assoc($product_result)) { ?>
                    <div class="product-container card" data-category="laptops" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                        ...
                        <div class="product-details">
                            ...
                            <p class="category"><strong>Category:</strong> Laptops > <?php echo htmlspecialchars($row['brand_name']); ?></p>
                        ...
                        <div class="product-right">
                            ...
                            <?php } ?>
                            <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'product')">
            ...
            <div class="endpara">
                <p>Compact enough to carry with you, yet versatile enough to run demanding applications,
                    a laptop is the best tool for doing serious work or play at home and on the road.</p>
                <p>We offer Core i7, i5, i3 Laptops at Best Prices in Pakistan - Dell - HP - Acer - Lenovo - Apple</p>
            </div>
        </div>
    </div>
    ...
    <div id="productPopup" class="popup" data-category="laptops" data-id="<?php echo htmlspecialchars($row['id']); ?>">
        <div class="popup-content">
            ...
            <div class="aside">
                ...
                <button class="add-to-cart" onclick="addToCart(0, 'product')">
                    <i class="fa-solid fa-cart-shopping"></i> Add To Cart
                </button>
