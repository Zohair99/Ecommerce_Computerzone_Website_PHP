...
$title = 'Processors Prices in Pakistan';
...
$sort_order = "processors.price ASC";

if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "processors.price DESC";
            break;
        case "low":
            $sort_order = "processors.price ASC";
            break;
        case "rating":
            $sort_order = "processors.review DESC";
            break;
        default:
            $sort_order = "processors.price ASC";
    }
...
if (!empty($selected_processor)) {
    $where_clauses[] = "processors_brand.processorsbrand = ?";
    $params[] = $selected_processor;
    $types .= 's';
}

if (!empty($selected_brand)) {
    $where_clauses[] = "processors_store.processorsstore = ?";
    $params[] = $selected_brand;
    $types .= 's';
}

if (!empty($searchQuery)) {
    $where_clauses[] = "processors.name LIKE ?";
    $params[] = '%' . $searchQuery . '%';
    $types .= 's';
}
...
$total_products_query = "SELECT COUNT(*) as total
                             FROM processors
                             INNER JOIN processors_brand ON processors.processor = processors_brand.processorsbrand
                             INNER JOIN processors_store ON processors.brand = processors_store.processorsstore"
    . $where_clause_string;
...
$product_query = "SELECT processors.*,
                     processors_brand.processorsbrand AS processor_name,
                     processors_store.processorsstore AS brand_name
                   FROM processors
                   INNER JOIN processors_brand ON processors.processor = processors_brand.processorsbrand
                   INNER JOIN processors_store ON processors.brand = processors_store.processorsstore"
    . $where_clause_string
...
<body>
    ...
    <nav class="breadcrumb">
        <ul>
            ...
            <li>/</li>
            <li><a href="processor.php" class="active">Processors</a></li>
        </ul>
    </nav>
    ...
    <div class="ppcontainer">
        <aside class="sidebar">
            <div class="filter-category">
                <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Processors</button>
                <ul class="filter-list" style="display: none;">
                    <?php
                    // Query for processor categories/stores
                    $store_query = "SELECT DISTINCT processorsstore FROM processors_store ORDER BY processorsstore ASC";
                    ...
                    if (!$store_result) {
                        ...
                        while ($row = mysqli_fetch_assoc($store_result)) {
                            $original_value = $row['processorsstore'];
            ...
            <div class="filter-category">
                ...
                <ul class="filter-list" style="display: none;">
                    <?php
                    // Query for brands of processors
                    $brand_query = "SELECT DISTINCT processorsbrand FROM processors_brand ORDER BY processorsbrand ASC";
                    ...
                    if (!$brand_result) {
                        ...
                        while ($row = mysqli_fetch_assoc($brand_result)) {
                            $original_value = $row['processorsbrand'];
                            $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                            $url_value = urlencode($original_value);
                            // Using 'processor' GET parameter to filter by brand, as per the PHP logic
        ...
        <div class="main-content">
            <div class="product-grid">
                <div class="mainheading">
                    <h3 id="pos-heading">Processors<?php echo $selected_brand ? " >> " . htmlspecialchars($selected_brand) : ''; ?></h3>
                </div>
                <div class="pp">
                    <p>Shop a wide selection of Processors in Pakistan including various models at Czone.com.pk</p>
                </div>
                ...
                <?php while ($row = mysqli_fetch_assoc($product_result)): ?>
                    <div class="product-container card" data-category="processors" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                        ...
                        <div class="product-details">
                            ...
                            <p class="category"><strong>Category:</strong> Processors > <?php echo htmlspecialchars($row['brand_name']); ?></p>
                        ...
                        <div class="product-right">
                            ...
                            <?php endif; ?>
                            <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'processors')">
            ...
            <div class="endpara">
                <p>Intel designs advanced, high-performance processors for every usage, including enterprise-scale servers, IoT devices, laptops, desktops, workstations, and mobile devices. Find the technology you need here.</p>
                <p>Whether you’re working or playing, you’ll get outstanding performance, incredible gaming, and amazing value with a multicore desktop processor from AMD. Buy now.</p>
            </div>
        </div>
    </div>
