...
function getRedirectUrl($baseUrl, $currentParams)
{
    ...
    if (!empty($currentParams['processor'])) { // This now refers to tabletacc_brand (e.g., specific type of accessory brand)
...
// Updated title for Tablet Accessories
$title = 'Tablet Accessories Price in Pakistan | Cases | Keyboards | Stylus';
...
// Changed table name to 'tabletacc' in sort order clauses
$sort_order = "tabletacc.price ASC";
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "tabletacc.price DESC";
            break;
        case "low":
            $sort_order = "tabletacc.price ASC";
            break;
        case "rating":
            $sort_order = "tabletacc.review DESC";
            break;
        default:
            $sort_order = "tabletacc.price ASC";
    }
...
$selected_brand = isset($_GET['brand']) ? $_GET['brand'] : ''; // This refers to tabletacc_store
$selected_processor = isset($_GET['processor']) ? urldecode($_GET['processor']) : ''; // This refers to tabletacc_brand
...
if (!empty($selected_brand)) {
    ...
    $where_clauses[] = "tabletacc_store.tabletaccstore = ?";
...
if (!empty($selected_processor)) {
    ...
    $where_clauses[] = "tabletacc_brand.tabletaccbrand = ?";
...
if (!empty($searchQuery)) {
    ...
    $where_clauses[] = "tabletacc.name LIKE ?";
    $params[] = '%' . $searchQuery . '%';
    $types .= 's';
}
...
$total_products_query = "SELECT COUNT(*) as total
                         FROM tabletacc
                         INNER JOIN tabletacc_store ON tabletacc.brand = tabletacc_store.tabletaccstore
                         INNER JOIN tabletacc_brand ON tabletacc.processor = tabletacc_brand.tabletaccbrand"
    . $where_clause_string; 
...
$product_query = "SELECT tabletacc.*, tabletacc_store.tabletaccstore AS brand_name, tabletacc_brand.tabletaccbrand AS processor_name
                  FROM tabletacc
                  INNER JOIN tabletacc_store ON tabletacc.brand = tabletacc_store.tabletaccstore
                  INNER JOIN tabletacc_brand ON tabletacc.processor = tabletacc_brand.tabletaccbrand"
    . $where_clause_string
...
<nav class="breadcrumb">
    <ul>
        ...
        <li>/</li>
        <li><a href="tabletaccessories.php" class="active">Tablet Accessories</a></li> </ul>
</nav>

<div class="ppcontainer">
    <aside class="sidebar">
        <div class="filter-category">
            <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Tablet Accessories</button>
            <ul class="filter-list" style="display: none;">
                <?php
                // Changed table and column names for store categories
                $store_query = "SELECT DISTINCT tabletaccstore FROM tabletacc_store ORDER BY tabletaccstore ASC";
                ...
                if (!$store_result) {
                    ...
                    while ($row = mysqli_fetch_assoc($store_result)) {
                        $original_value = $row['tabletaccstore'];
        ...
        <div class="filter-category">
            ...
            <ul class="filter-list" style="display: none;">
                ...
                $brand_query = "SELECT DISTINCT tabletaccbrand FROM tabletacc_brand ORDER BY created_at DESC";
                ...
                if (!$brand_result) {
                    ...
                    while ($row = mysqli_fetch_assoc($brand_result)) {
                        $original_value = $row['tabletaccbrand'];
                        $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                        $url_value = urlencode($original_value);
    ...
    <div class="main-content">
        <div class="product-grid">
            <div class="mainheading">
                <h3 id="mouse-heading">Tablet Accessories <?php echo $selected_brand ? " >> " . htmlspecialchars($selected_brand) : ''; ?></h3> </div>
            <div class="pp">
                <p>Shop a wide selection of Tablet Accessories in Pakistan including various models at Czone.com.pk</p> </div>
            ...
            <?php while ($row = mysqli_fetch_assoc($product_result)): {
                // Ensure 'brand' and 'processor' keys exist, falling back to 'N/A' if not
                $brand = isset($row['brand_name']) ? htmlspecialchars($row['brand_name']) : 'N/A'; // From tabletacc_store
                $processor = isset($row['processor_name']) ? htmlspecialchars($row['processor_name']) : 'N/A'; // From tabletacc_brand
            } ?>
                <div class="product-container card" data-category="tabletacc" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                    ...
                    <div class="product-details">
                        ...
                        <p class="category"><strong>Category:</strong> Tablet Accessories > <?php echo $brand; ?></p> <p class="code"><strong>Code:</strong> <?php echo htmlspecialchars($row['code']); ?></p>
                    ...
                    <div class="product-right">
                        ...
                        <?php endif; ?>
                        <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'tabletacc')"> <i class="fa-solid fa-cart-shopping"></i> Add To Cart
                        </button>
        ...
        <div class="endpara">
            <p>Get precise, responsive control with our wide range of Tablet Accessories for all your computing needs.</p> <p>From everyday use to professional applications, we've got you covered with trusted brands.</p> <p>Enjoy high-performance accessories, all at competitive prices in Pakistan.</p> </div>
    </div>
</div>

<div id="productPopup" class="popup" data-category="tabletacc"> <div class="popup-content">
        ...
        <div class="aside">
            ...
            <button class="add-to-cart" onclick="addToCart(0, 'tabletacc')"> <i class="fa-solid fa-cart-shopping"></i> Add To Cart
            </button>
