...
function getRedirectUrl($baseUrl, $currentParams)
{
    ...
    if (!empty($currentParams['processor'])) { // This now refers to usedproduct_brand
...
// Updated title for Used Products
$title = 'Used Product Price in Pakistan | Used Products';
...
// Changed table name to 'usedproduct' in sort order clauses
$sort_order = "usedproduct.price ASC";
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "usedproduct.price DESC";
            break;
        case "low":
            $sort_order = "usedproduct.price ASC";
            break;
        case "rating":
            $sort_order = "usedproduct.review DESC";
            break;
        default:
            $sort_order = "usedproduct.price ASC";
    }
...
$selected_brand = isset($_GET['brand']) ? $_GET['brand'] : ''; // This refers to usedproduct_store
$selected_processor = isset($_GET['processor']) ? urldecode($_GET['processor']) : ''; // This refers to usedproduct_brand
...
if (!empty($selected_brand)) {
    ...
    $where_clauses[] = "usedproduct_store.usedproductstore = ?";
...
if (!empty($selected_processor)) {
    ...
    $where_clauses[] = "usedproduct_brand.usedproductbrand = ?";
...
if (!empty($searchQuery)) {
    ...
    $where_clauses[] = "usedproduct.name LIKE ?";
    $params[] = '%' . $searchQuery . '%';
    $types .= 's';
}
...
$total_products_query = "SELECT COUNT(*) as total
                         FROM usedproduct
                         INNER JOIN usedproduct_store ON usedproduct.brand = usedproduct_store.usedproductstore
                         INNER JOIN usedproduct_brand ON usedproduct.processor = usedproduct_brand.usedproductbrand"
    . $where_clause_string;
...
$product_query = "SELECT usedproduct.*, usedproduct_store.usedproductstore AS brand_name, usedproduct_brand.usedproductbrand AS processor_name
                  FROM usedproduct
                  INNER JOIN usedproduct_store ON usedproduct.brand = usedproduct_store.usedproductstore
                  INNER JOIN usedproduct_brand ON usedproduct.processor = usedproduct_brand.usedproductbrand"
    . $where_clause_string
...
<nav class="breadcrumb">
    <ul>
        ...
        <li>/</li>
        <li><a href="usedproducts.php" class="active">Used Products</a></li>
    </ul>
</nav>
...
<div class="ppcontainer">
    <aside class="sidebar">
        <div class="filter-category">
            <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Used Products </button>
            <ul class="filter-list" style="display: none;">
                <?php
                // Changed table and column names for store categories
                $store_query = "SELECT DISTINCT usedproductstore FROM usedproduct_store ORDER BY usedproductstore ASC";
                ...
                if (!$store_result) {
                    ...
                    while ($row = mysqli_fetch_assoc($store_result)) {
                        $original_value = $row['usedproductstore'];
        ...
        <div class="filter-category">
            ...
            <ul class="filter-list" style="display: none;">
                ...
                $brand_query = "SELECT DISTINCT usedproductbrand FROM usedproduct_brand ORDER BY created_at DESC";
                ...
                if (!$brand_result) {
                    ...
                    while ($row = mysqli_fetch_assoc($brand_result)) {
                        $original_value = $row['usedproductbrand'];
                        $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                        $url_value = urlencode($original_value);
    ...
    <div class="main-content">
        <div class="product-grid">
            <div class="mainheading">
                <h3 id="mouse-heading">Used Products <?php echo $selected_brand ? " >> " . htmlspecialchars($selected_brand) : ''; ?></h3>
            </div>
            <div class="pp">
                <p>Shop a wide selection of Used Products in Pakistan including various models at Czone.com.pk</p>
            </div>
            ...
            <?php while ($row = mysqli_fetch_assoc($product_result)): {
                ...
                <div class="product-container card" data-category="usedproduct" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                    ...
                    <div class="product-details">
                        ...
                        <p class="category"><strong>Category:</strong> Used Products > <?php echo $brand; ?></p>
                    ...
                    <div class="product-right">
                        ...
                        <?php endif; ?>
                        <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'usedproduct')"> <i class="fa-solid fa-cart-shopping"></i> Add To Cart
                        </button>
        ...
        <div class="endpara">
            <p>Get precise, responsive control with our wide range of Used Products for all your computing needs.</p>
            <p>From everyday use to professional applications, we've got you covered with trusted brands.</p>
            <p>Enjoy high-performance Used Products, all at competitive prices in Pakistan.</p>
        </div>
    </div>
</div>

<div id="productPopup" class="popup" data-category="usedproduct">
    <div class="popup-content">
        ...
        <div class="aside">
            ...
            <button class="add-to-cart" onclick="addToCart(0, 'usedproduct')"> <i class="fa-solid fa-cart-shopping"></i> Add To Cart
            </button>
