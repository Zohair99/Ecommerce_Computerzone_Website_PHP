...
$title = 'Memory Cards Prices in Pakistan - SD / MicroSD Memory Cards';
...
$sort_order = "memorycards.price ASC";
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "memorycards.price DESC";
            break;
        case "low":
            $sort_order = "memorycards.price ASC";
            break;
        case "rating":
            $sort_order = "memorycards.review DESC";
            break;
        default:
            $sort_order = "memorycards.price ASC";
    }
...
if (!empty($selected_brand)) {
    $where_clauses[] = "memorycards_store.memorycardsstore = ?";
    $params[] = $selected_brand;
    $types .= 's';
}

if (!empty($selected_processor)) {
    $where_clauses[] = "memorycards_brand.memorycardsbrand = ?";
    $params[] = $selected_processor;
    $types .= 's';
}

if (!empty($searchQuery)) {
    $where_clauses[] = "memorycards.name LIKE ?";
    $params[] = '%' . $searchQuery . '%';
    $types .= 's';
}
...
$total_products_query = "SELECT COUNT(*) as total
                         FROM memorycards
                         INNER JOIN memorycards_store ON memorycards.brand = memorycards_store.memorycardsstore
                         INNER JOIN memorycards_brand ON memorycards.processor = memorycards_brand.memorycardsbrand"
                         . $where_clause_string;
...
$product_query = "SELECT memorycards.*, memorycards_store.memorycardsstore AS brand_name, memorycards_brand.memorycardsbrand AS processor_name
                  FROM memorycards
                  INNER JOIN memorycards_store ON memorycards.brand = memorycards_store.memorycardsstore
                  INNER JOIN memorycards_brand ON memorycards.processor = memorycards_brand.memorycardsbrand"
                  . $where_clause_string
...
?>
    ...
    <nav class="breadcrumb">
        <ul>
            ...
            <li>/</li>
            <li><a href="memorycard.php" class="active">Memory Cards</a></li>
        </ul>
    </nav>
    ...
    <div class="ppcontainer">
        <!-- Left Sidebar -->
        <aside class="sidebar">
            <!-- Memory Card Brand Filter -->
            <div class="filter-category">
                <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Brands</button>
                <ul class="filter-list" style="display: none;">
                    <?php
                    $store_query = "SELECT DISTINCT memorycardsstore FROM memorycards_store ORDER BY memorycardsstore ASC";
                    ...
                    if (!$store_result) {
                        ...
                        while ($row = mysqli_fetch_assoc($store_result)) {
                            $original_value = $row['memorycardsstore'];
            ...
            <!-- Memory Card Capacity Filter -->
            <div class="filter-category">
                <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Capacity</button>
                <ul class="filter-list" style="display: none;">
                    <?php
                    $brand_query = "SELECT DISTINCT memorycardsbrand FROM memorycards_brand ORDER BY created_at DESC";
                    ...
                    if (!$brand_result) {
                        ...
                        while ($row = mysqli_fetch_assoc($brand_result)) {
                            $original_value = $row['memorycardsbrand'];
                            $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                            $url_value = urlencode($original_value);
        ...
        <div class="main-content">
            <div class="product-grid">
                <div class="mainheading">
                    <h3 id="memory-heading">Memory Cards<?php echo $selected_brand ? " >> " . htmlspecialchars($selected_brand) : ''; ?></h3>
                </div>
                <div class="pp">
                    <p>Shop a wide selection of high-performance Memory Cards in Pakistan including SD, microSD, and CF cards at Czone.com.pk</p>
                </div>
                ...
                <?php while ($row = mysqli_fetch_assoc($product_result)): {
                        ...
                        $capacity = isset($row['processor']) ? htmlspecialchars($row['processor']) : 'N/A';
                    } ?>
                    <div class="product-container card" data-category="memory-cards" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                        ...
                        <div class="product-details">
                            ...
                            <p class="category"><strong>Category:</strong> Memory Cards > <?php echo $brand; ?></p>
                        ...
                        <div class="product-right">
                            ...
                            <?php endif; ?>
                            <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'memorycards')">
            ...
            <!-- End Paragraph -->
            <div class="endpara">
                <p>Expand your device's storage with our premium selection of high-speed, reliable memory cards for all your needs.</p>
                <p>From high-capacity SD cards for cameras to durable microSD cards for smartphones, we offer trusted brands like SanDisk, Samsung, Kingston, and Lexar.</p>
                <p>Get fast read/write speeds, waterproof and temperature-proof designs, and great value at competitive prices in Pakistan.</p>
            </div>
        </div>
    </div>

    <!-- Product Popup -->
    <div id="productPopup" class="popup" data-category="memory-cards">
        <div class="popup-content">
            ...
            <div class="aside">
                ...
                <button class="add-to-cart" onclick="addToCart(0, 'memorycards')">
                    <i class="fa-solid fa-cart-shopping"></i> Add To Cart
                </button>
