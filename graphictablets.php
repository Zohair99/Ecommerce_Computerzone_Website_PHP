...
$title = 'Graphic Tablets Prices in Pakistan - Wacom';
...
$sort_order = "graphictablets.price ASC";
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "graphictablets.price DESC";
            break;
        case "low":
            $sort_order = "graphictablets.price ASC";
            break;
        case "rating":
            $sort_order = "graphictablets.review DESC";
            break;
    }
}

$selected_brand = isset($_GET['brand']) ? mysqli_real_escape_string($conn, $_GET['brand']) : '';
$selected_processor = isset($_GET['processor']) ? mysqli_real_escape_string($conn, urldecode($_GET['processor'])) : '';

$base_product_query = "SELECT graphictablets.*, graphictablets_store.graphictabletsstore AS brand_name, graphictablets_brand.graphictabletsbrand AS processor_name
                       FROM graphictablets
                       INNER JOIN graphictablets_store ON graphictablets.brand = graphictablets_store.graphictabletsstore
                       INNER JOIN graphictablets_brand ON graphictablets.processor = graphictablets_brand.graphictabletsbrand";
...
if (!empty($selected_brand)) {
    $conditions[] = "graphictablets_store.graphictabletsstore = '" . $selected_brand . "'";
}
if (!empty($selected_processor)) {
    $conditions[] = "graphictablets_brand.graphictabletsbrand = '" . $selected_processor . "'";
}
if (!empty($searchQuery)) {
    $conditions[] = "graphictablets.name LIKE '%" . $searchQuery . "%'";
}
...
?>
    ...
    <nav class="breadcrumb">
        <ul>
            ...
            <li>/</li>
            <li><a href="graphictablets.php" class="active">Graphic Tablets</a></li>
        </ul>
    </nav>
    ...
    <div class="ppcontainer">
        <!-- Left Sidebar -->
        <aside class="sidebar">
            <!-- Graphic Tablets Filter -->
            <div class="filter-category">
                <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Graphic Tablets</button>
                <ul class="filter-list" style="display: none;">
                    <?php
                    $store_query = "SELECT DISTINCT graphictabletsstore FROM graphictablets_store ORDER BY graphictabletsstore ASC";
                    ...
                    if (!$store_result) {
                        ...
                        while ($row = mysqli_fetch_assoc($store_result)) {
                            $original_value = $row['graphictabletsstore'];
            ...
            <div class="filter-category">
                ...
                <ul class="filter-list" style="display: none;">
                    <?php
                    $brand_query = "SELECT DISTINCT graphictabletsbrand FROM graphictablets_brand ORDER BY graphictabletsbrand ASC";
                    ...
                    if (!$brand_result) {
                        ...
                        while ($row = mysqli_fetch_assoc($brand_result)) {
                            $original_value = $row['graphictabletsbrand'];
                            $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                            $url_value = urlencode($original_value);
        ...
        <div class="main-content">
            <div class="product-grid">
                <div class="mainheading">
                    <h3 id="graphictablets-heading">Graphic Tablets<?php echo $selected_brand ? " >> " . htmlspecialchars($selected_brand) : ''; ?></h3>
                </div>
                <div class="pp">
                    <p>Shop a wide selection of Graphic Tablets in Pakistan including various models at Czone.com.pk</p>
                </div>
                ...
                <?php while ($row = mysqli_fetch_assoc($product_result)): {
                    ...
                    <div class="product-container card" data-category="graphictablets" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                        ...
                        <div class="product-details">
                            ...
                            <p class="category"><strong>Category:</strong> Graphic Tablets > <?php echo $brand; ?></p>
                        ...
                        <div class="product-right">
                            ...
                            <?php endif; ?>
                            <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'graphictablets')">
            ...
            <!-- End Paragraph -->
            <div class="endpara">
                <p>Unleash your creativity with our premium selection of graphic tablets, perfect for digital artists, designers, and creative professionals.</p>
                <p>From beginner-friendly models to professional-grade tablets, we offer trusted brands like Wacom, Huion, and XP-Pen at competitive prices in Pakistan.</p>
                <p>Experience natural drawing feel, pressure sensitivity, and precise control for all your digital artwork and design projects.</p>
            </div>
        </div>
    </div>

    <!-- Product Popup -->
    <div id="productPopup" class="popup" data-category="graphictablets">
        <div class="popup-content">
            ...
            <div class="aside">
                ...
                <button class="add-to-cart" onclick="addToCart(0, 'graphictablets')">
                    <i class="fa-solid fa-cart-shopping"></i> Add To Cart
                </button>
