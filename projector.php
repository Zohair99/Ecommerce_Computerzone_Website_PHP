...
$title = 'Projectors Prices in Pakistan';
...
$sort_order = "projectors.price ASC";
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "projectors.price DESC";
            break;
        case "low":
            $sort_order = "projectors.price ASC";
            break;
        case "rating":
            $sort_order = "projectors.review DESC";
            break;
        default:
            $sort_order = "projectors.price ASC";
    }
...
if (!empty($selected_processor)) {
    $where_clauses[] = "projectors_brand.projectorsbrand = ?";
    $params[] = $selected_processor;
    $types .= 's';
}

if (!empty($selected_brand)) {
    $where_clauses[] = "projectors_store.projectorsstore = ?";
    $params[] = $selected_brand;
    $types .= 's';
}

if (!empty($searchQuery)) {
    $where_clauses[] = "projectors.name LIKE ?";
    $params[] = '%' . $searchQuery . '%';
    $types .= 's';
}
...
$total_products_query = "SELECT COUNT(*) as total
    FROM projectors
    INNER JOIN projectors_brand ON projectors.processor = projectors_brand.projectorsbrand
    INNER JOIN projectors_store ON projectors.brand = projectors_store.projectorsstore"
    . $where_clause_string;
...
$product_query = "SELECT projectors.*,
    projectors_brand.projectorsbrand AS processor_name,
    projectors_store.projectorsstore AS brand_name
    FROM projectors
    INNER JOIN projectors_brand ON projectors.processor = projectors_brand.projectorsbrand
    INNER JOIN projectors_store ON projectors.brand = projectors_store.projectorsstore"
    . $where_clause_string
...
<body>
    ...
    <nav class="breadcrumb">
        <ul>
            ...
            <li>/</li>
            <li><a href="projector.php" class="active">Projectors</a></li>
        </ul>
    </nav>
    ...
    <div class="ppcontainer">
        <aside class="sidebar">
            <div class="filter-category">
                <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Projectors </button>
                <ul class="filter-list" style="display: none;">
                    <?php
                    // Query for projector store categories
                    $store_query = "SELECT DISTINCT projectorsstore FROM projectors_store ORDER BY projectorsstore ASC";
                    ...
                    if (!$store_result) {
                        ...
                        while ($row = mysqli_fetch_assoc($store_result)) {
                            $original_value = $row['projectorsstore'];
            ...
            <div class="filter-category">
                ...
                <ul class="filter-list" style="display: none;">
                    <?php
                    // Query for projector brands
                    $brand_query = "SELECT DISTINCT projectorsbrand FROM projectors_brand ORDER BY projectorsbrand ASC";
                    ...
                    if (!$brand_result) {
                        ...
                        while ($row = mysqli_fetch_assoc($brand_result)) {
                            $original_value = $row['projectorsbrand'];
                            $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                            $url_value = urlencode($original_value);
                            // Using 'processor' GET parameter to filter by brand
        ...
        <div class="main-content">
            <div class="product-grid">
                <div class="mainheading">
                    <h3 id="pos-heading">Projectors<?php echo $selected_brand ? " >> " . htmlspecialchars($selected_brand) : ''; ?></h3>
                </div>
                <div class="pp">
                    <p>Shop a wide selection of Projectors in Pakistan including various models at Czone.com.pk</p>
                </div>
                ...
                <?php while ($row = mysqli_fetch_assoc($product_result)): ?>
                    <div class="product-container card" data-category="projectors" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                        ...
                        <div class="product-details">
                            ...
                            <p class="category"><strong>Category:</strong> Projectors > <?php echo htmlspecialchars($row['brand_name']); ?></p>
                        ...
                        <div class="product-right">
                            ...
                            <?php endif; ?>
                            <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'projectors')">
            ...
            <div class="endpara">
                <p>Explore high-quality projectors designed for every setting — from home entertainment and business presentations to educational use and large-scale events. Discover the perfect projection solution to suit your needs.</p>
                <p>Whether you're setting up a home theater or delivering professional presentations, you'll enjoy sharp visuals, vibrant colors, and great value with our range of projectors. Shop now for the latest models.</p>
            </div>
        </div>
    </div>
