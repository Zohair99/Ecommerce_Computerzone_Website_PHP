...
function getRedirectUrl($baseUrl, $currentParams) {
    ...
    if (!empty($currentParams['solid_disk'])) {
        $params['solid_disk'] = $currentParams['solid_disk'];
    }
    if (!empty($currentParams['c_storage'])) {
        $params['c_storage'] = $currentParams['c_storage'];
    }
...
$title = 'SSD Solid State Drive Prices in Pakistan';
...
$sort_order = "ssd_data.price ASC";
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "ssd_data.price DESC";
            break;
        case "low":
            $sort_order = "ssd_data.price ASC";
            break;
        case "rating":
            $sort_order = "ssd_data.review DESC";
            break;
        default:
            $sort_order = "ssd_data.price ASC";
    }
...
$solid_disk_filter = isset($_GET['solid_disk']) ? $_GET['solid_disk'] : '';
$c_storage_filter = isset($_GET['c_storage']) ? $_GET['c_storage'] : '';

$where_clauses = [];
$params = [];
$types = '';

if (!empty($solid_disk_filter)) {
    $where_clauses[] = "ssd.solid_disk = ?";
    $params[] = $solid_disk_filter;
    $types .= 's';
}

if (!empty($c_storage_filter)) {
    $where_clauses[] = "s_capacity.c_storage = ?";
    $params[] = $c_storage_filter;
    $types .= 's';
}

if (!empty($searchQuery)) {
    $where_clauses[] = "ssd_data.name LIKE ?";
    $params[] = '%' . $searchQuery . '%';
    $types .= 's';
}
...
$total_products_query = "SELECT COUNT(*) as total
                         FROM ssd_data
                         INNER JOIN ssd ON ssd_data.sd = ssd.solid_disk
                         INNER JOIN s_capacity ON ssd_data.store = s_capacity.c_storage"
                         . $where_clause_string;
...
$product_query = "SELECT ssd_data.*, ssd.solid_disk AS solid_disk_name, s_capacity.c_storage AS c_storage_name
                  FROM ssd_data
                  INNER JOIN ssd ON ssd_data.sd = ssd.solid_disk
                  INNER JOIN s_capacity ON ssd_data.store = s_capacity.c_storage"
                  . $where_clause_string
...
?>
    ...
    <nav class="breadcrumb">
        <ul>
            ...
            <li>/</li>
            <li><a href="ssd.php" class="active">Solid State Drives<?php echo $solid_disk_filter ? " >> " . htmlspecialchars($solid_disk_filter) : ''; ?></a></li>
        </ul>
    </nav>
    ...
    <div class="ppcontainer">
        ...
        <aside class="sidebar">
            ...
            <div class="filter-category">
                <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> SSD Types</button>
                <ul class="filter-list" style="display: none;">
                    <?php
                    $graphic_query = "SELECT DISTINCT solid_disk FROM ssd ORDER BY solid_disk ASC";
                    $graphic_result = mysqli_query($conn, $graphic_query);

                    if (!$graphic_result) {
                        echo "<li>Error fetching SSD types: " . htmlspecialchars(mysqli_error($conn)) . "</li>";
                    } else {
                        while ($row = mysqli_fetch_assoc($graphic_result)) {
                            $original_value = $row['solid_disk'];
                            $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                            $url_value = urlencode($original_value);
                            $is_active = (isset($_GET['solid_disk'])) && urldecode($_GET['solid_disk']) === $original_value ? 'active' : '';
                            echo "<li class='$is_active'><a href='?solid_disk=$url_value'>$display_value</a></li>";
                        }
                    }
                    ?>
                </ul>
            ...
            <!-- Capacity Filter -->
            <div class="filter-category">
                <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Capacity</button>
                <ul class="filter-list" style="display: none;">
                    <?php
                    $size_query = "SELECT DISTINCT c_storage FROM s_capacity ORDER BY c_storage ASC";
                    $size_result = mysqli_query($conn, $size_query);

                    if (!$size_result) {
                        echo "<li>Error fetching capacities: " . htmlspecialchars(mysqli_error($conn)) . "</li>";
                    } else {
                        while ($row = mysqli_fetch_assoc($size_result)) {
                            $original_value = $row['c_storage'];
                            $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                            $url_value = urlencode($original_value);
                            $is_active = (isset($_GET['c_storage'])) && urldecode($_GET['c_storage']) === $original_value ? 'active' : '';
                            echo "<li class='$is_active'><a href='?c_storage=$url_value'>$display_value</a></li>";
                        }
                    }
                    ?>
                </ul>
        ...
        <div class="main-content">
            <div class="product-grid">
                <div class="mainheading">
                    <h3 id="ssd-heading">Solid State Drives<?php echo $solid_disk_filter ? " >> " . htmlspecialchars($solid_disk_filter) : ''; ?></h3>
                </div>
                <div class="pp">
                    <p>Shop a wide selection of Solid State Drives in Pakistan including various models at Czone.com.pk</p>
                </div>
                ...
                <?php while ($row = mysqli_fetch_assoc($product_result)): {
                        $solid_disk = isset($row['solid_disk']) ? htmlspecialchars($row['solid_disk']) : 'N/A';
                        $c_storage = isset($row['c_storage']) ? htmlspecialchars($row['c_storage']) : 'N/A';
                    } ?>
                    <div class="product-container card" data-category="ssd" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                        ...
                        <div class="product-details">
                            ...
                            <p class="category"><strong>Type:</strong> <?php echo $solid_disk; ?></p>
                            <p class="category"><strong>Capacity:</strong> <?php echo $c_storage; ?></p>
                        ...
                        <div class="product-right">
                            ...
                            <?php endif; ?>
                            <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'ssd_data')">
            ...
            <!-- End Paragraph -->
            <div class="endpara">
                <p>Boost your system's speed and performance with high-speed SSDs that load applications and files in seconds. Perfect for gaming, editing, or everyday use, SSDs offer faster boot times, better reliability, and lower power consumption.</p>
                <p>Explore top brands like Samsung, Kingston, Crucial, and WD at the best prices in Pakistan. Whether you need an NVMe drive for blazing-fast performance or a SATA SSD for reliable storage upgrades, we have the perfect solution for your needs.</p>
                <p>Speed up your world — switch to SSD today and experience the difference in performance.</p>
            </div>
        </div>
    </div>

    <!-- Product Popup -->
    <div id="productPopup" class="popup" data-category="ssd">
        <div class="popup-content">
            ...
            <div class="aside">
                ...
                <button class="add-to-cart" onclick="addToCart(0, 'ssd_data')">
                    <i class="fa-solid fa-cart-shopping"></i> Add To Cart
                </button>
