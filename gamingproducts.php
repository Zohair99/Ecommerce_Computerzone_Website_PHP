...
function getRedirectUrl($baseUrl, $currentParams, $searchQuery = '')
{
    ...
    if (!empty($currentParams['processor'])) { // Changed 'processor' to 'type' or 'category' if more fitting for gaming products
...
$title = 'Gaming Products Prices in Pakistan';
...
$sort_order = "gamingproducts.price ASC";
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "gamingproducts.price DESC";
            break;
        case "low":
            $sort_order = "gamingproducts.price ASC";
            break;
        case "rating":
            $sort_order = "gamingproducts.review DESC";
            break;
    }
}

$selected_brand = isset($_GET['brand']) ? mysqli_real_escape_string($conn, $_GET['brand']) : '';
$selected_type = isset($_GET['processor']) ? mysqli_real_escape_string($conn, urldecode($_GET['processor'])) : ''; // Using 'type' for consistency with other categories, if 'processor' isn't explicitly for CPU type in gaming products

$base_product_query = "SELECT gamingproducts.*, gamingproducts_store.gamingproductsstore AS brand_name, gamingproducts_brand.gamingproductsbrand AS type_name
                       FROM gamingproducts
                       INNER JOIN gamingproducts_store ON gamingproducts.brand = gamingproducts_store.gamingproductsstore
                       INNER JOIN gamingproducts_brand ON gamingproducts.processor = gamingproducts_brand.gamingproductsbrand";
...
if (!empty($selected_brand)) {
    $conditions[] = "gamingproducts_store.gamingproductsstore = '" . $selected_brand . "'";
}
if (!empty($selected_type)) {
    $conditions[] = "gamingproducts_brand.gamingproductsbrand = '" . $selected_type . "'";
}
if (!empty($searchQuery)) {
    $conditions[] = "gamingproducts.name LIKE '%" . $searchQuery . "%'";
}
...
<nav class="breadcrumb">
    <ul>
        ...
        <li>/</li>
        <li><a href="gamingproducts.php" class="active">Gaming Products</a></li>
    </ul>
</nav>
...
<div class="ppcontainer">
    <!-- Left Sidebar -->
    <aside class="sidebar">
        <!-- Gaming Products Filter -->
        <div class="filter-category">
            <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Gaming Products</button>
            <ul class="filter-list" style="display: none;">
                <?php
                $store_query = "SELECT DISTINCT gamingproductsstore FROM gamingproducts_store ORDER BY gamingproductsstore ASC";
                ...
                if (!$store_result) {
                    ...
                    while ($row = mysqli_fetch_assoc($store_result)) {
                        $original_value = $row['gamingproductsstore'];
        ...
        <div class="filter-category">
            ...
            <ul class="filter-list" style="display: none;">
                <?php
                $brand_query = "SELECT DISTINCT gamingproductsbrand FROM gamingproducts_brand ORDER BY created_at DESC";
                ...
                if (!$brand_result) {
                    ...
                    while ($row = mysqli_fetch_assoc($brand_result)) {
                        $original_value = $row['gamingproductsbrand'];
                        $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                        $url_value = urlencode($original_value);
    ...
    <div class="main-content">
        <div class="product-grid">
            <div class="mainheading">
                <h3 id="gaming-heading">Gaming Products<?php echo $selected_type ? " >> " . htmlspecialchars($selected_type) : ''; ?></h3>
            </div>
            <div class="pp">
                <p>Shop a wide selection of Gaming Products in Pakistan including various models at Czone.com.pk</p>
            </div>
            ...
            <?php while ($row = mysqli_fetch_assoc($product_result)): {
                ...
                <div class="product-container card" data-category="gaming-products" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                    ...
                    <div class="product-details">
                        ...
                        <p class="category"><strong>Category:</strong> Gaming Products > <?php echo $brand; ?></p>
                    ...
                    <div class="product-right">
                        ...
                        <?php endif; ?>
                        <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'gamingproducts')">
        ...
        <!-- End Paragraph -->
        <div class="endpara">
            <p>Get consistent, high-quality gaming experience every time with our wide range of Gaming Products.</p>
            <p>From casual gaming to professional esports, we've got you covered with trusted brands like Razer, Logitech, and SteelSeries.</p>
...
<!-- Product Popup -->
<div id="productPopup" class="popup" data-category="gaming-products">
    <div class="popup-content">
        ...
        <div class="aside">
            ...
            <button class="add-to-cart" onclick="addToCart(0, 'gamingproducts')">
                <i class="fa-solid fa-cart-shopping"></i> Add To Cart
            </button>
