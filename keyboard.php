...
$title = 'Keyboard Prices in Pakistan - Computer Keyboard - Desktop...';
...
$sort_order = "keyboard.price ASC";
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "keyboard.price DESC";
            break;
        case "low":
            $sort_order = "keyboard.price ASC";
            break;
        case "rating":
            $sort_order = "keyboard.review DESC";
            break;
        default:
            $sort_order = "keyboard.price ASC";
    }
...
if (!empty($selected_brand)) {
    $where_clauses[] = "keyboard_store.keyboardstore = ?";
    $params[] = $selected_brand;
    $types .= 's';
}

if (!empty($selected_processor)) {
    $where_clauses[] = "keyboard_brand.keyboardbrand = ?";
    $params[] = $selected_processor;
    $types .= 's';
}

if (!empty($searchQuery)) {
    $where_clauses[] = "keyboard.name LIKE ?";
    $params[] = '%' . $searchQuery . '%';
    $types .= 's';
}
...
$total_products_query = "SELECT COUNT(*) as total
                         FROM keyboard
                         INNER JOIN keyboard_store ON keyboard.brand = keyboard_store.keyboardstore
                         INNER JOIN keyboard_brand ON keyboard.processor = keyboard_brand.keyboardbrand"
                         . $where_clause_string;
...
$product_query = "SELECT keyboard.*, keyboard_store.keyboardstore AS brand_name, keyboard_brand.keyboardbrand AS processor_name
                  FROM keyboard
                  INNER JOIN keyboard_store ON keyboard.brand = keyboard_store.keyboardstore
                  INNER JOIN keyboard_brand ON keyboard.processor = keyboard_brand.keyboardbrand"
                  . $where_clause_string
...
<nav class="breadcrumb">
    <ul>
        ...
        <li>/</li>
        <li><a href="keyboard.php" class="active">Keyboard</a></li>
    </ul>
</nav>
...
<div class="ppcontainer">
    <!-- Left Sidebar -->
    <aside class="sidebar">
        <!-- Keyboard Store Filter -->
        <div class="filter-category">
            <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Keyboard</button>
            <ul class="filter-list" style="display: none;">
                <?php
                $store_query = "SELECT DISTINCT keyboardstore FROM keyboard_store ORDER BY keyboardstore ASC";
                ...
                if (!$store_result) {
                    ...
                    while ($row = mysqli_fetch_assoc($store_result)) {
                        $original_value = $row['keyboardstore'];
        ...
        <div class="filter-category">
            ...
            <ul class="filter-list" style="display: none;">
                <?php
                $brand_query = "SELECT DISTINCT keyboardbrand FROM keyboard_brand ORDER BY created_at DESC";
                ...
                if (!$brand_result) {
                    ...
                    while ($row = mysqli_fetch_assoc($brand_result)) {
                        $original_value = $row['keyboardbrand'];
                        $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                        $url_value = urlencode($original_value);
    ...
    <div class="main-content">
        <div class="product-grid">
            <div class="mainheading">
                <h3 id="keyboard-heading">Keyboard<?php echo $selected_brand ? " >> " . htmlspecialchars($selected_brand) : ''; ?></h3>
            </div>
            <div class="pp">
                <p>Shop a wide selection of Keyboards in Pakistan including mechanical, membrane, and gaming keyboards at Czone.com.pk</p>
            </div>
            ...
            <?php while ($row = mysqli_fetch_assoc($product_result)): {
                ...
                <div class="product-container card" data-category="keyboard" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                    ...
                    <div class="product-details">
                        ...
                        <p class="category"><strong>Category:</strong> Keyboard > <?php echo $brand; ?></p>
                    ...
                    <div class="product-right">
                        ...
                        <?php endif; ?>
                        <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'keyboard')">
        ...
        <!-- End Paragraph -->
        <div class="endpara">
            <p>Enhance your typing experience with our premium selection of keyboards for all needs and budgets.</p>
            <p>From mechanical keyboards for gamers to slim wireless keyboards for professionals, we offer trusted brands like Razer, Logitech, Corsair, and HyperX.</p>
            <p>Find the perfect keyboard with responsive keys, durable construction, and great value at competitive prices in Pakistan.</p>
        </div>
    </div>
</div>

<!-- Product Popup -->
<div id="productPopup" class="popup" data-category="keyboard">
    <div class="popup-content">
        ...
        <div class="aside">
            ...
            <button class="add-to-cart" onclick="addToCart(0, 'keyboard')">
                <i class="fa-solid fa-cart-shopping"></i> Add To Cart
            </button>
