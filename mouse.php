...
$title = 'Gaming Mouse Prices in Pakistan - Desktop Mouse - PC Mouse';
...
$sort_order = "mouse.price ASC";
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "mouse.price DESC";
            break;
        case "low":
            $sort_order = "mouse.price ASC";
            break;
        case "rating":
            $sort_order = "mouse.review DESC";
            break;
        default:
            $sort_order = "mouse.price ASC";
    }
...
if (!empty($selected_brand)) {
    $where_clauses[] = "mouse_store.mousestore = ?";
    $params[] = $selected_brand;
    $types .= 's';
}

if (!empty($selected_processor)) {
    $where_clauses[] = "mouse_brand.mousebrand = ?";
    $params[] = $selected_processor;
    $types .= 's';
}

if (!empty($searchQuery)) {
    $where_clauses[] = "mouse.name LIKE ?";
    $params[] = '%' . $searchQuery . '%';
    $types .= 's';
}
...
$total_products_query = "SELECT COUNT(*) as total
                         FROM mouse
                         INNER JOIN mouse_store ON mouse.brand = mouse_store.mousestore
                         INNER JOIN mouse_brand ON mouse.processor = mouse_brand.mousebrand"
                         . $where_clause_string;
...
$product_query = "SELECT mouse.*, mouse_store.mousestore AS brand_name, mouse_brand.mousebrand AS processor_name
                  FROM mouse
                  INNER JOIN mouse_store ON mouse.brand = mouse_store.mousestore
                  INNER JOIN mouse_brand ON mouse.processor = mouse_brand.mousebrand"
                  . $where_clause_string
...
?>
    ...
    <nav class="breadcrumb">
        <ul>
            ...
            <li>/</li>
            <li><a href="mouse.php" class="active">Mouse</a></li>
        </ul>
    </nav>
    ...
    <div class="ppcontainer">
        <!-- Left Sidebar -->
        <aside class="sidebar">
            <!-- Mouse Products Filter -->
            <div class="filter-category">
                <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Mouse</button>
                <ul class="filter-list" style="display: none;">
                    <?php
                    $store_query = "SELECT DISTINCT mousestore FROM mouse_store ORDER BY mousestore ASC";
            ...
            <div class="filter-category">
                ...
                <ul class="filter-list" style="display: none;">
                    <?php
                    $brand_query = "SELECT DISTINCT mousebrand FROM mouse_brand ORDER BY created_at DESC";
        ...
        <div class="main-content">
            <div class="product-grid">
                <div class="mainheading">
                    <h3 id="mouse-heading">Mouse<?php echo $selected_brand ? " >> " . htmlspecialchars($selected_brand) : ''; ?></h3>
                </div>
                <div class="pp">
                    <p>Shop a wide selection of Mouse in Pakistan including various models at Czone.com.pk</p>
                </div>
                ...
                <?php while ($row = mysqli_fetch_assoc($product_result)): {
                    ...
                    <div class="product-container card" data-category="mouse" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                        ...
                        <div class="product-details">
                            ...
                            <p class="category"><strong>Category:</strong> Mouse > <?php echo $brand; ?></p>
                        ...
                        <div class="product-right">
                            ...
                            <?php endif; ?>
                            <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'mouse')">
            ...
            <!-- End Paragraph -->
            <div class="endpara">
                <p>Get precise, responsive control with our wide range of Mice for all your computing needs.</p>
                <p>From everyday use to professional gaming, we've got you covered with trusted brands like Logitech, Razer, and SteelSeries.</p>
                <p>Enjoy ergonomic designs and high-performance sensors, all at competitive prices in Pakistan.</p>
            </div>
        </div>
    </div>

    <!-- Product Popup -->
    <div id="productPopup" class="popup" data-category="mouse">
        <div class="popup-content">
            ...
            <div class="aside">
                ...
                <button class="add-to-cart" onclick="addToCart(0, 'mouse')">
                    <i class="fa-solid fa-cart-shopping"></i> Add To Cart
                </button>
