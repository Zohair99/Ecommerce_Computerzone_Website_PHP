...
function getRedirectUrl($baseUrl, $currentParams) {
    ...
    if (!empty($currentParams['processor'])) $params['processor'] = $currentParams['processor']; // Add processor param for laptopaccessories
...
$title = 'Laptop accessories Prices in Pakistan | Laptops';
...
$sort_order = "laptopacc.price ASC";
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "laptopacc.price DESC";
            break;
        case "low":
            $sort_order = "laptopacc.price ASC";
            break;
        case "rating":
            $sort_order = "laptopacc.review DESC";
            break;
    }
}

// Filters for laptop accessories
$selected_brand = isset($_GET['brand']) ? mysqli_real_escape_string($conn, $_GET['brand']) : '';
$selected_processor = isset($_GET['processor']) ? mysqli_real_escape_string($conn, urldecode($_GET['processor'])) : ''; // Adjusted variable name for clarity

// Base queries
$base_product_query = "SELECT laptopacc.*, laptopacc_store.laptopacc AS brand_name, brand_store.brand AS processor_name
                       FROM laptopacc
                       INNER JOIN laptopacc_store ON laptopacc.brand = laptopacc_store.laptopacc
                       INNER JOIN brand_store ON laptopacc.processor = brand_store.brand";
...
if (!empty($selected_brand)) {
    $conditions[] = "laptopacc_store.laptopacc = '" . $selected_brand . "'";
}
if (!empty($selected_processor)) {
    $conditions[] = "brand_store.brand = '" . $selected_processor . "'";
}
if (!empty($searchQuery)) {
    $conditions[] = "laptopacc.name LIKE '%" . $searchQuery . "%'";
}
...
<nav class="breadcrumb">
    <ul>
        ...
        <li>/</li>
        <li><a href="laptopaccessories.php" class="active">Laptop Accessories</a></li>
    </ul>
</nav>
...
<div class="ppcontainer">
    <!-- Left Sidebar -->
    <aside class="sidebar">
        <!-- Laptop Accessories Filter -->
        <div class="filter-category">
            <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Laptop Accessories</button>
            <ul class="filter-list" style="display: none;">
                <?php
                $brand_query = "SELECT DISTINCT laptopacc FROM laptopacc_store ORDER BY laptopacc ASC";
        ...
        <div class="filter-category">
            ...
            <ul class="filter-list" style="display: none;">
                <?php
                $processor_query = "SELECT DISTINCT brand FROM brand_store ORDER BY created_at DESC";
    ...
    <div class="main-content">
        <div class="product-grid">
            <div class="mainheading">
                <h3 id="laptop-heading">Laptop Accessories<?php echo $selected_brand ? " >> " . htmlspecialchars($selected_brand) : ''; ?></h3>
            </div>
            <div class="pp">
                <p>Shop a wide selection of Laptop Accessories in Pakistan including various models at Czone.com.pk</p>
            </div>
            ...
            <?php while ($row = mysqli_fetch_assoc($product_result)): {
                ...
                <div class="product-container card" data-category="laptop-accessories" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                    ...
                    <div class="product-details">
                        ...
                        <p class="category"><strong>Category:</strong> Laptop Accessories > <?php echo $processor; ?></p>
                    ...
                    <div class="product-right">
                        ...
                        <?php } ?>
                        <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'laptopacc')">
        ...
        <!-- End Paragraph -->
        <div class="endpara">
            <p>Laptop accessories are essential for enhancing the functionality and comfort of your device. Whether you need a protective case, an external mouse, a portable keyboard, or additional storage, the right accessories can make your laptop experience more efficient and enjoyable. Accessories like laptop stands, cooling pads, and docking stations can also improve ergonomics and prevent overheating, helping your laptop run smoothly for longer periods.</p>
            <p>We offer a wide range of laptop accessories from top brands like Logitech, Anker, and Targus, all at affordable prices in Pakistan to complement your laptop perfectly.</p>
        </div>
    </div>
</div>

<!-- Product Popup -->
<div id="productPopup" class="popup" data-category="laptop-accessories">
    <div class="popup-content">
        ...
        <div class="aside">
            ...
            <button class="add-to-cart" onclick="addToCart(0, 'laptopacc')">
                <i class="fa-solid fa-cart-shopping"></i> Add To Cart
            </button>
