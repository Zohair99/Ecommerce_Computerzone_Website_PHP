...
$title = 'Streaming Devices | Media Players | TV Devices Price in Pakistan';
...
$sort_order = "tvstream.price ASC"; // Changed table name
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "tvstream.price DESC"; // Changed table name
            break;
        case "low":
            $sort_order = "tvstream.price ASC"; // Changed table name
            break;
        case "rating":
            $sort_order = "tvstream.review DESC"; // Changed table name
            break;
        default:
            $sort_order = "tvstream.price ASC"; // Changed table name
    }
...
if (!empty($selected_brand)) {
    $where_clauses[] = "tvstream_store.tvstreamstore = ?"; // Changed table and column name
    $params[] = $selected_brand;
    $types .= 's';
}

if (!empty($selected_processor)) {
    $where_clauses[] = "tvstream_brand.tvstreambrand = ?"; // Changed table and column name
    $params[] = $selected_processor;
    $types .= 's';
}

if (!empty($searchQuery)) {
    $where_clauses[] = "tvstream.name LIKE ?"; // Changed table name
    $params[] = '%' . $searchQuery . '%';
    $types .= 's';
}
...
$total_products_query = "SELECT COUNT(*) as total
                         FROM tvstream
                         INNER JOIN tvstream_store ON tvstream.brand = tvstream_store.tvstreamstore
                         INNER JOIN tvstream_brand ON tvstream.processor = tvstream_brand.tvstreambrand"
    . $where_clause_string;
...
$product_query = "SELECT tvstream.*, tvstream_store.tvstreamstore AS brand_name, tvstream_brand.tvstreambrand AS processor_name
                  FROM tvstream
                  INNER JOIN tvstream_store ON tvstream.brand = tvstream_store.tvstreamstore
                  INNER JOIN tvstream_brand ON tvstream.processor = tvstream_brand.tvstreambrand"
    . $where_clause_string
...
<nav class="breadcrumb">
    <ul>
        ...
        <li>/</li>
        <li><a href="tvdevices-streaming.php" class="active">TV Devices | Streaming Media Players</a></li>
    </ul>
</nav>
    ...
    <aside class="sidebar">
        <div class="filter-category">
            ...
            <ul class="filter-list" style="display: none;">
                <?php
                $store_query = "SELECT DISTINCT tvstreamstore FROM tvstream_store ORDER BY tvstreamstore ASC"; // Changed table name
                ...
                if (!$store_result) {
                    ...
                    while ($row = mysqli_fetch_assoc($store_result)) {
                        $original_value = $row['tvstreamstore']; // Changed column name
        ...
        <div class="filter-category">
            <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Price Range</button>
            <ul class="filter-list" style="display: none;">
                <?php
                $brand_query = "SELECT DISTINCT tvstreambrand FROM tvstream_brand ORDER BY created_at DESC"; // Changed table name
                ...
                if (!$brand_result) {
                    ...
                    while ($row = mysqli_fetch_assoc($brand_result)) {
                        $original_value = $row['tvstreambrand']; // Changed column name
                        $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                        $url_value = urlencode($original_value);
    ...
    <div class="main-content">
        <div class="product-grid">
            <div class="mainheading">
                <h3 id="mouse-heading">TV Devices | Streaming Media Players<?php echo $selected_brand ? " >> " . htmlspecialchars($selected_brand) : ''; ?></h3>
            </div>
            <div class="pp">
                <p>Shop a wide selection of TV Devices | Streaming Media Players in Pakistan including various models at Czone.com.pk</p>
            </div>
            ...
            <?php while ($row = mysqli_fetch_assoc($product_result)): {
                ...
                <div class="product-container card" data-category="tvstream" data-id="<?php echo htmlspecialchars($row['id']); ?>"> <?php if (!empty($row['offer'])): ?>
                    ...
                    <div class="product-details">
                        ...
                        <p class="category"><strong>Category:</strong> TV Devices | Streaming Media Players > <?php echo $brand; ?></p>
                    ...
                    <div class="product-right">
                        ...
                        <?php endif; ?>
                        <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'tvstream')"> <i class="fa-solid fa-cart-shopping"></i> Add To Cart
                        </button>
        ...
        <div class="endpara">
            <p>Get precise, responsive control with our wide range of TV Devices | Streaming Media Players for all your computing needs.</p>
            <p>From everyday use to professional streaming, we've got you covered with trusted brands.</p>
            <p>Enjoy high-performance streaming, all at competitive prices in Pakistan.</p>
        </div>
    </div>
</div>

<div id="productPopup" class="popup" data-category="tvstream">
    <div class="popup-content">
        ...
        <div class="aside">
            ...
            <button class="add-to-cart" onclick="addToCart(0, 'tvstream')"> <i class="fa-solid fa-cart-shopping"></i> Add To Cart
            </button>
