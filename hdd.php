...
function getRedirectUrl($baseUrl, $currentParams)
{
    ...
    if (!empty($currentParams['hard_disk'])) {
        $params['hard_disk'] = $currentParams['hard_disk'];
    }
...
$title = 'Hard Drive Prices in Pakistan - HDD Portable - Surveillance - SATA';
...
$sort_order = "hdd_data.price ASC";
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "hdd_data.price DESC";
            break;
        case "low":
            $sort_order = "hdd_data.price ASC";
            break;
        case "rating":
            $sort_order = "hdd_data.review DESC";
            break;
        default:
            $sort_order = "hdd_data.price ASC"; // Default if sort is invalid
    }
}

$hard_disk_filter = isset($_GET['hard_disk']) ? $_GET['hard_disk'] : '';
...
if (!empty($hard_disk_filter)) {
    $where_clauses[] = "hdd.hard_disk = ?";
    $params[] = $hard_disk_filter;
    $types .= 's';
}

if (!empty($c_storage_filter)) {
    $where_clauses[] = "capacity.c_storage = ?";
    $params[] = $c_storage_filter;
    $types .= 's';
}

if (!empty($searchQuery)) {
    $where_clauses[] = "hdd_data.name LIKE ?";
    $params[] = '%' . $searchQuery . '%';
    $types .= 's';
}
...
$total_products_query = "SELECT COUNT(*) as total
                         FROM hdd_data
                         INNER JOIN hdd ON hdd_data.hd = hdd.hard_disk
                         INNER JOIN capacity ON hdd_data.store = capacity.c_storage"
    . $where_clause_string;
...
$product_query = "SELECT hdd_data.*, hdd.hard_disk, capacity.c_storage
                  FROM hdd_data
                  INNER JOIN hdd ON hdd_data.hd = hdd.hard_disk
                  INNER JOIN capacity ON hdd_data.store = capacity.c_storage"
    . $where_clause_string
...
<nav class="breadcrumb">
    <ul>
        ...
        <li>/</li>
        <li><a href="hdd.php" class="active">Hard Disk Drive</a></li>
    </ul>
</nav>
...
<div class="ppcontainer">
    <aside class="sidebar">
        <div class="filter-category">
            <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Hard Disk Types</button>
            <ul class="filter-list" style="display: none;">
                <?php
                $hdd_query = "SELECT DISTINCT hard_disk FROM hdd ORDER BY hard_disk ASC";
                ...
                if (!$hdd_result) {
                    ...
                    while ($row = mysqli_fetch_assoc($hdd_result)) {
                        $original_value = $row['hard_disk'];
                        $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                        $url_value = urlencode($original_value);
                        $is_active = (isset($_GET['hard_disk'])) && urldecode($_GET['hard_disk']) === $original_value ? 'active' : '';
                        echo "<li class='$is_active'><a href='?hard_disk=$url_value'>$display_value</a></li>";
                    }
                }
                ?>
            </ul>
        ...
        <div class="filter-category">
            ...
            <ul class="filter-list" style="display: none;">
                <?php
                $capacity_query = "SELECT DISTINCT c_storage FROM capacity ORDER BY c_storage ASC";
                $capacity_result = mysqli_query($conn, $capacity_query);

                if (!$capacity_result) {
                    echo "<li>Error fetching capacity options: " . htmlspecialchars(mysqli_error($conn)) . "</li>";
                } else {
    ...
    <div class="main-content">
        <div class="product-grid">
            <div class="mainheading">
                <h3 id="hdd-heading">Hard Disk Drive<?php echo $hard_disk_filter ? " >> " . htmlspecialchars($hard_disk_filter) : ''; ?></h3>
            </div>
            <div class="pp">
                <p>Shop a wide selection of Hard Disk Drives in Pakistan including various models at Czone.com.pk</p>
            </div>
            ...
            <?php while ($row = mysqli_fetch_assoc($product_result)): {
                    $hard_disk = isset($row['hard_disk']) ? htmlspecialchars($row['hard_disk']) : 'N/A';
                    $c_storage = isset($row['c_storage']) ? htmlspecialchars($row['c_storage']) : 'N/A';
                } ?>
                <div class="product-container card" data-category="hdd" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                    ...
                    <div class="product-details">
                        ...
                        <p class="category"><strong>Category:</strong> Hard Drives > <?php echo $hard_disk; ?></p>
                    ...
                    <div class="product-right">
                        ...
                        <?php endif; ?>
                        <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'hdd_data')">
        ...
        <!-- End Paragraph -->
        <div class="endpara">
            <p>Store all your important files, photos, videos, and software with our reliable and spacious HDDs. Ideal for personal and professional use, these hard drives offer large capacities at affordable prices without compromising performance.</p>
            <p>Choose from trusted brands like Seagate, Western Digital, Toshiba, and more in Pakistan, available in various capacities to suit your storage needs.</p>
            <p>More space, more possibilities — keep everything within reach with our high-quality hard disk drives.</p>
        </div>
    </div>
</div>

<!-- Product Popup -->
<div id="productPopup" class="popup" data-category="hdd">
    <div class="popup-content">
        ...
        <div class="aside">
            ...
            <button class="add-to-cart" onclick="addToCart(0, 'hdd_data')">
                <i class="fa-solid fa-cart-shopping"></i> Add To Cart
            </button>
