...
function getRedirectUrl($baseUrl, $currentParams)
{
    ...
    if (!empty($currentParams['monitor'])) {
        $params['monitor'] = $currentParams['monitor'];
    }
    if (!empty($currentParams['size'])) {
        $params['size'] = $currentParams['size'];
    }
...
$title = 'LCD/LED Monitors Prices in Pakistan 2025 - Best Deals & Offers';
...
$sort_order = "monitor.price ASC";
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "monitor.price DESC";
            break;
        case "low":
            $sort_order = "monitor.price ASC";
            break;
        case "rating":
            $sort_order = "monitor.review DESC";
            break;
        default:
            $sort_order = "monitor.price ASC";
    }
...
$selected_monitor_type = isset($_GET['monitor']) ? $_GET['monitor'] : '';
...
if (!empty($selected_monitor_type)) {
    $where_clauses[] = "lcdmonitor.monitor = ?";
    $params[] = $selected_monitor_type;
    $types .= 's';
}

if (!empty($selected_size)) {
    $where_clauses[] = "REPLACE(REPLACE(inches.size, '\"', ''), \"'\", '') = ?";
    $params[] = str_replace(['"', "'"], '', $selected_size);
    $types .= 's';
}

if (!empty($searchQuery)) {
    $where_clauses[] = "monitor.name LIKE ?";
    $params[] = '%' . $searchQuery . '%';
    $types .= 's';
}
...
$total_products_query = "SELECT COUNT(*) as total
                         FROM monitor
                         INNER JOIN lcdmonitor ON monitor.lcdmonitor = lcdmonitor.monitor
                         INNER JOIN inches ON monitor.inches = inches.size"
    . $where_clause_string;
...
$product_query = "SELECT monitor.*, lcdmonitor.monitor AS monitor_type, inches.size AS monitor_size
                  FROM monitor
                  INNER JOIN lcdmonitor ON monitor.lcdmonitor = lcdmonitor.monitor
                  INNER JOIN inches ON monitor.inches = inches.size"
    . $where_clause_string
...
<nav class="breadcrumb">
    <ul>
        ...
        <li>/</li>
        <li><a href="monitor.php" class="active">LCD/LED Monitors</a></li>
    </ul>
</nav>
...
<div class="ppcontainer">
    ...
    <aside class="sidebar">
        ...
        <div class="filter-category">
            <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Monitor Types</button>
            <ul class="filter-list" style="display: none;">
                <?php
                $graphic_query = "SELECT DISTINCT monitor FROM lcdmonitor";
                ...
                if (!$graphic_result) {
                    ...
                    while ($row = mysqli_fetch_assoc($graphic_result)) {
                        $original_value = $row['monitor'];
                        $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                        $url_value = urlencode($original_value);
                        $is_active = (isset($_GET['monitor']) && urldecode($_GET['monitor']) === $original_value) ? 'active' : '';
                        echo "<li class='$is_active'><a href='?monitor=$url_value'>$display_value</a></li>";
                    }
                }
                ?>
            </ul>
        ...
        <!-- Display Size Filter -->
        <div class="filter-category">
            <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Display Size</button>
            <ul class="filter-list" style="display: none;">
                <?php
                $size_query = "SELECT DISTINCT size FROM inches";
                $size_result = mysqli_query($conn, $size_query);

                if (!$size_result) {
                    echo "<li>Error fetching size options: " . htmlspecialchars(mysqli_error($conn)) . "</li>";
                } else {
                    while ($row = mysqli_fetch_assoc($size_result)) {
                        $original_value = $row['size'];
                        $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                        $size_url_param = str_replace('"', '', $row['size']);
                        $is_active = (isset($_GET['size']) && urldecode($_GET['size']) === $size_url_param) ? 'active' : '';
                        echo "<li class='$is_active'><a href='?size=" . urlencode($size_url_param) . "'>$display_value</a></li>";
                    }
                }
                ?>
            </ul>
    ...
    <div class="main-content">
        <div class="product-grid">
            <div class="mainheading">
                <h3 id="monitor-heading">LCD/LED Monitors<?php echo $selected_monitor_type ? " >> " . htmlspecialchars($selected_monitor_type) : ''; ?></h3>
            </div>
            <div class="pp">
                <p>Shop a wide selection of high-quality LCD/LED Monitors in Pakistan including various models at Czone.com.pk</p>
            </div>
            ...
            <?php while ($row = mysqli_fetch_assoc($product_result)): ?>
                <div class="product-container card" data-category="monitor" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                    ...
                    <div class="product-details">
                        ...
                        <p class="category"><strong>Category:</strong> LCD/LED Monitors > <?php echo isset($row['lcdmonitor']) ? htmlspecialchars($row['lcdmonitor']) : 'N/A'; ?></p>
                    ...
                    <div class="product-right">
                        ...
                        <?php endif; ?>
                        <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'monitor')">
        ...
        <!-- End Paragraph -->
        <div class="endpara">
            <p>Upgrade your visual experience with our premium selection of LCD/LED monitors offering crystal-clear displays and vibrant colors.</p>
            <p>From high-resolution gaming monitors to professional-grade displays, we feature top brands like Dell, HP, Samsung, and LG at competitive prices in Pakistan.</p>
            <p>Find the perfect monitor with the right size, resolution, and features to enhance your productivity and entertainment.</p>
        </div>
    </div>
</div>

<!-- Product Popup -->
<div id="productPopup" class="popup" data-category="monitor">
    <div class="popup-content">
        ...
        <div class="aside">
            ...
            <button class="add-to-cart" onclick="addToCart(0, 'monitor')">
                <i class="fa-solid fa-cart-shopping"></i> Add To Cart
            </button>
