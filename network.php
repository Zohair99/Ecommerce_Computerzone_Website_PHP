...
function getRedirectUrl($baseUrl, $currentParams) {
    ...
    if (!empty($currentParams['np'])) {
        $params['np'] = $currentParams['np'];
    }
    if (!empty($currentParams['type'])) {
        $params['type'] = $currentParams['type'];
    }
...
$title = 'Network Products Prices in Pakistan';
...
$sort_order = "network.price ASC";

if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case "high":
            $sort_order = "network.price DESC";
            break;
        case "low":
            $sort_order = "network.price ASC";
            break;
        case "rating":
            $sort_order = "network.review DESC";
            break;
        default:
            $sort_order = "network.price ASC";
    }
...
$selected_np = isset($_GET['np']) ? $_GET['np'] : '';
$selected_type = isset($_GET['type']) ? urldecode($_GET['type']) : '';

$where_clauses = [];
$params = [];
$types = '';

if (!empty($selected_np)) {
    $where_clauses[] = "np.network = ?";
    $params[] = $selected_np;
    $types .= 's';
}

if (!empty($selected_type)) {
    $where_clauses[] = "type.n_type = ?";
    $params[] = $selected_type;
    $types .= 's';
}

if (!empty($searchQuery)) {
    $where_clauses[] = "network.name LIKE ?";
    $params[] = '%' . $searchQuery . '%';
    $types .= 's';
}
...
$total_products_query = "SELECT COUNT(*) as total
                         FROM network
                         INNER JOIN np ON network.np = np.network
                         INNER JOIN type ON network.type = type.n_type"
                         . $where_clause_string;
...
$product_query = "SELECT network.*, np.network AS np_name, type.n_type AS type_name
                  FROM network
                  INNER JOIN np ON network.np = np.network
                  INNER JOIN type ON network.type = type.n_type"
                  . $where_clause_string
...
?>
    ...
    <div class="ppcontainer">
        <!-- Left Sidebar -->
        <aside class="sidebar">
            <!-- Network Products Filter -->
            <div class="filter-category">
                <button class="filter-toggle"><i class="fa-solid fa-caret-down"></i> Network Products</button>
                <ul class="filter-list" style="display: none;">
                    <?php
                    $type_query = "SELECT DISTINCT n_type FROM type ORDER BY n_type ASC";
                    ...
                    if (!$type_result) {
                        ...
                        while ($row = mysqli_fetch_assoc($type_result)) {
                            $original_value = $row['n_type'];
                            $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                            $url_value = urlencode($original_value);
                            $is_active = (isset($_GET['type']) && urldecode($_GET['type']) === $original_value) ? 'active' : '';
                            echo "<li class='$is_active'><a href='?type=$url_value'>$display_value</a></li>";
                        }
                    }
                    ?>
                </ul>
            ...
            <div class="filter-category">
                ...
                <ul class="filter-list" style="display: none;">
                    <?php
                    $np_query = "SELECT DISTINCT network FROM np ORDER BY network ASC";
                    ...
                    if (!$np_result) {
                        ...
                        while ($row = mysqli_fetch_assoc($np_result)) {
                            $original_value = $row['network'];
                            $display_value = htmlspecialchars($original_value, ENT_QUOTES);
                            $url_value = urlencode($original_value);
                            $is_active = (isset($_GET['np']) && urldecode($_GET['np']) === $original_value) ? 'active' : '';
                            echo "<li class='$is_active'><a href='?np=$url_value'>$display_value</a></li>";
                        }
                    }
                    ?>
                </ul>
        ...
        <div class="main-content">
            <div class="product-grid">
                <div class="mainheading">
                    <h3 id="network-heading">Network<?php echo $selected_type ? " >> " . htmlspecialchars($selected_type) : ''; ?></h3>
                </div>
                <div class="pp">
                    <p>Shop a wide selection of Network products in Pakistan including various models at Czone.com.pk</p>
                </div>
                ...
                <?php while ($row = mysqli_fetch_assoc($product_result)): {
                    ...
                    <div class="product-container card" data-category="network" data-id="<?php echo htmlspecialchars($row['id']); ?>">
                        ...
                        <div class="product-details">
                            ...
                            <p class="category"><strong>Network:</strong> <?php echo $np; ?></p>
                            <p class="category"><strong>Type:</strong> <?php echo $n_type; ?></p>
                        ...
                        <div class="product-right">
                            ...
                            <?php endif; ?>
                            <button class="add-to-cart" onclick="addToCart(<?php echo htmlspecialchars($row['id']); ?>, 'network')">
            ...
            <!-- End Paragraph -->
            <div class="endpara">
                <p>Stay connected with fast and reliable networking solutions designed for homes, offices, and businesses. From routers and switches to Wi-Fi extenders and network cards, we offer everything you need to build a strong, stable connection.</p>
                <p>Choose top brands like TP-Link, D-Link, Cisco, and Ubiquiti at the best prices in Pakistan.</p>
                <p>Strong connection, seamless performance — power your network with confidence.</p>
            </div>
        </div>
    </div>
